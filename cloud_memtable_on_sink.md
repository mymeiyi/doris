...
普通 Stream Load 使用请求头 `cloud_memtable_sink_upload`，未传时默认 `true`。
HTTP Stream、Routine Load 和 Group Commit 内部写入使用 FE 动态配置
`cloud_stream_load_memtable_sink_upload`，默认 `true`。
仍需通过 `memtable_on_sink_node` 请求参数或已有 FE 默认配置开启 MemTable 前移。
新增配置不改变非 Cloud 导入。

直传需要参与导入的 BE 及 MetaService、Recycler 均支持当前分支的显式 Segment ID 格式。
不再通过 open 响应进行直传能力握手。
同一次导入中，同一 tablet 的模式在写入开始后固定，不能混用直传与文件转发；
同一 tablet 的不同导入可以使用不同路径。

### 1.2 Cloud 前移总流程

Sink BE 是执行导入 Sink 的 BE，目标 BE 是负责该 tablet
...
`cloud_stream_load_memtable_sink_upload` 默认配置。`async_mode` 的外层请求可在
...
以下步骤描述 Sink 上传路径；文件转发路径的调用关系见 2.2 至 2.4 节。
  ...
  → _append_data()
    → IndexStream::append_data()
      → _init_tablet_stream()                                             [该 tablet 首次消息]
        → TabletStream::init()
          → 创建 LoadStreamWriter
            ├─ CloudRowsetBuilder(engine.to_cloud(), ...)                 [Cloud]
            └─ RowsetBuilder(engine.to_local(), ...)                      [存算一体]
...
`CloudRowsetWriter`，但直接通过 `RowsetFactory` 创建，不经过目标端的 `CloudRowsetBuilder`，
见下一节。
...
异常、空 writer、重复结果和缺失结果。直传开关当前默认开启；性能收益需通过 RELEASE benchmark 验证。
...
Broker Load、Stream Load、delete sign、重插入和 compaction 前后结果。
...
`test_cloud_memtable_sink_upload_mow` 覆盖不共享 writer、存量删除状态、稀疏 ID、Sequence、跨 Sink
重复 key、packed 开/关、V2 索引、Stream/Broker Load、删除后低 Sequence 写入、compaction，
以及固定快照后并发写入并替换存量 Rowset、bitmap 计算失败事务不可见。
无 packed 的直传结果与非前移参考表比较；并发导入、compaction 和同表切换上传路径后显式检查重复 key。
`test_cloud_memtable_on_sink` 还覆盖 VARIANT 多 writer 元数据合并、后续新增子字段与 compaction 后读取，
以及普通 Stream Load 的 header/默认值不受 HTTP Stream 配置影响。
