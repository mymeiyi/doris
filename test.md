...
```text
distributed_compaction.is_distributed = true
distributed_compaction.task_count = actual_ranges
actual_ranges >= 2
distributed_compaction.worker_count >= 2
```

分布式指标集中在 `distributed_compaction` 对象内；本地 compaction 不展示该对象。分布式任务还包含：

- `job_id`：用于关联协调端与 worker 日志。
- `plan_time_us`：发现 worker、规划范围、分配任务及构造请求的耗时，不包含提交 RPC。
- `submit_rpc_time_us`：任务提交 RPC 的耗时。
- `bytes_read_from_peer`：各 worker 从其他 BE 读取的字节数之和。
- `local_read_time_us`、`remote_read_time_us`、`peer_read_time_us`：各 worker 任务读取耗时之和。
- `worker_cpu_time_us`：各 worker 任务 CPU 时间之和。

各项耗时存在并行重叠，不能直接相加作为顶层 `cost_time_ms`。通用的行数、大小、本地和远端读取字节数、merge 耗时、内存峰值和列组统计仍在顶层。
...
- `W`：`distributed_compaction.worker_count`。
...
| --- | --- | ---: | --- | --- |
| P0-1 | DUP BIGINT，均匀 | 4～8 GiB | 并行关闭 | profile 中不存在 `distributed_compaction` 字段，成功 |
...
额外建立一个 MOW + cluster key 表作为回退用例，预期 profile 中不存在 `distributed_compaction` 字段，不纳入并行加速比。
...
要求 `success=true`、profile 中不存在 `distributed_compaction` 字段。
...
- `success=true`、`distributed_compaction.is_distributed=true`。
- `distributed_compaction.task_count` 等于日志中的 `actual_ranges`。
- `distributed_compaction.worker_count = min(live BE, actual ranges)`。
