...
public class MetaServiceProxy {
    ...
    public Cloud.GetInstanceResponse getInstance(Cloud.GetInstanceRequest request)
        ...
        try {
            ...
            if (MetricRepo.isInit && Config.isCloudMode()) {
                CloudMetrics.META_SERVICE_RPC_LATENCY.getOrAdd(methodName).update(latency);
            }
    ...
    public static class MetaServiceClientWrapper {
        ...
        public <Response> Response executeRequest(String methodName,
            ...
            for (long tried = 1; tried <= maxRetries; tried++) {
                ...
                try {
                    ...
                    // Track retry metrics on success (if retries occurred)
                    if (retryCount > 0 && MetricRepo.isInit && Config.isCloudMode()) {
                        CloudMetrics.META_SERVICE_RPC_RETRY.getOrAdd(methodName).increase(retryCount);
                        CloudMetrics.META_SERVICE_RPC_ALL_RETRY.increase(retryCount);
                    }
                    ...
                    if (!shouldRetry || tried >= maxRetries) {
                        // Track retry metrics when all retries exhausted
                        if (retryCount > 0 && MetricRepo.isInit && Config.isCloudMode()) {
                            CloudMetrics.META_SERVICE_RPC_RETRY.getOrAdd(methodName).increase(retryCount);
                            CloudMetrics.META_SERVICE_RPC_ALL_RETRY.increase(retryCount);
                        }
        ...
        public <Response> Response executeRequest(Function<MetaServiceClient, Response> function)
                throws RpcException {
            return executeRequest("unknown", function);
        }
    }
     ...
     * Tracks: total calls, failures, latency, retries, request/response sizes.
     */
    private <Request, Response> Response executeWithMetrics(
            String methodName,
            Request request,
            Function<MetaServiceClient, Response> function) throws RpcException {
        ...
        // Track total calls and request size
        if (MetricRepo.isInit && Config.isCloudMode()) {
            CloudMetrics.META_SERVICE_RPC_TOTAL.getOrAdd(methodName).increase(1L);
            CloudMetrics.META_SERVICE_RPC_ALL_TOTAL.increase(1L);
        }
        ...
        try {
            ...
            if (MetricRepo.isInit && Config.isCloudMode()) {
                // Track success metrics: latency and response size
                long latency = System.currentTimeMillis() - startTime;
                CloudMetrics.META_SERVICE_RPC_LATENCY.getOrAdd(methodName).update(latency);
            }
            return response;
            ...
            if (MetricRepo.isInit && Config.isCloudMode()) {
                ...
                // Also track latency for failed requests
                long latency = System.currentTimeMillis() - startTime;
                CloudMetrics.META_SERVICE_RPC_LATENCY.getOrAdd(methodName).update(latency);
            }
            throw e;
    ...
    public Future<Cloud.GetVersionResponse> getVisibleVersionAsync(Cloud.GetVersionRequest request)
            throws RpcException {
        long startTime = System.currentTimeMillis();
        String methodName = "getVisibleVersionAsync";
        MetaServiceClient client = null;
    ...
    public Cloud.RemoveDeleteBitmapUpdateLockResponse removeDeleteBitmapUpdateLock(
            ...
            throws RpcException {
        return executeWithMetrics("removeDeleteBitmapUpdateLock", request,
    ...
    public Cloud.AlterObjStoreInfoResponse alterObjStoreInfo(Cloud.AlterObjStoreInfoRequest request)
            throws RpcException {
        return executeWithMetrics("alterObjStoreInfo", request, (client) -> client.alterObjStoreInfo(request));
    }

    public Cloud.AlterObjStoreInfoResponse alterStorageVault(Cloud.AlterObjStoreInfoRequest request)
            throws RpcException {
        return executeWithMetrics("alterStorageVault", request, (client) -> client.alterStorageVault(request));
    }

    public Cloud.FinishTabletJobResponse finishTabletJob(Cloud.FinishTabletJobRequest request)
            throws RpcException {
        return executeWithMetrics("finishTabletJob", request, (client) -> client.finishTabletJob(request));
    }

    public Cloud.GetRLTaskCommitAttachResponse
            getRLTaskCommitAttach(Cloud.GetRLTaskCommitAttachRequest request)
            throws RpcException {
        return executeWithMetrics("getRLTaskCommitAttach", request,
    ...
    public Cloud.ResetRLProgressResponse resetRLProgress(Cloud.ResetRLProgressRequest request)
            throws RpcException {
        return executeWithMetrics("resetRLProgress", request, (client) -> client.resetRLProgress(request));
    }

    public Cloud.ResetStreamingJobOffsetResponse resetStreamingJobOffset(Cloud.ResetStreamingJobOffsetRequest request)
            throws RpcException {
        return executeWithMetrics("resetStreamingJobOffset", request,
    ...
    public Cloud.GetObjStoreInfoResponse
        ...
        return executeWithMetrics("getObjStoreInfo", request, (client) -> client.getObjStoreInfo(request));
    }
    ...
    public Cloud.AbortTxnWithCoordinatorResponse
        ...
        return executeWithMetrics("abortTxnWithCoordinator", request,
    ...
    public Cloud.GetPrepareTxnByCoordinatorResponse
        ...
        return executeWithMetrics("getPrepareTxnByCoordinator", request,
    ...
    public Cloud.CreateInstanceResponse createInstance(Cloud.CreateInstanceRequest request) throws RpcException {
        return executeWithMetrics("createInstance", request, (client) -> client.createInstance(request));
    }
    ...
    public Cloud.GetStreamingTaskCommitAttachResponse getStreamingTaskCommitAttach(
        ...
        return executeWithMetrics("getStreamingTaskCommitAttach", request,
