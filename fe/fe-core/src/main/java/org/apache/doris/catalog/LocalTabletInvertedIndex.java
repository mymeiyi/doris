...
public class LocalTabletInvertedIndex extends TabletInvertedIndex {
    ...
    private Long2ObjectOpenHashMap<Long2ObjectOpenHashMap<Replica>> replicaMetaTable = new Long2ObjectOpenHashMap<>();
    ...
    public Map<TStorageMedium, TreeMultimap<Long, PartitionBalanceInfo>> buildPartitionInfoBySkew(
        ...
        try {
            ...
            // tablet id -> (backend id -> replica)
            for (Entry<Long2ObjectOpenHashMap<Replica>> tabletEntry : replicaMetaTable.long2ObjectEntrySet()) {
                long tabletId = tabletEntry.getLongKey();
                for (Entry<Replica> replicaEntry : tabletEntry.getValue().long2ObjectEntrySet()) {
                    long beId = replicaEntry.getLongKey();
    ...
    public Table<Long, Long, Replica> getReplicaMetaTable() {
        ...
        try {
            ...
            replicaMetaTable.entrySet().stream().forEach(tabletEntry -> {
                tabletEntry.getValue().entrySet().stream().forEach(replicaEntry -> {
