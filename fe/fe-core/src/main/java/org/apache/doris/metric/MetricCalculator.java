...
public class MetricCalculator extends TimerTask {
    ...
    private long lastMetaServiceCounter = -1;
    ...
    private void update() {
        ...
        if (lastTs == -1) {
            ...
            if (Config.isCloudMode()) {
                lastMetaServiceCounter = CloudMetrics.META_SERVICE_RPC_ALL_TOTAL.getValue();
            }
            initCloudMetrics();
            return;
        }
        ...
        if (Config.isCloudMode()) {
            ...
            double metaServiceRps = (double) (currentMetaServiceCounter - lastMetaServiceCounter) / interval;
            CloudMetrics.META_SERVICE_RPC_ALL_PER_SECOND.setValue(metaServiceRps < 0 ? 0.0 : metaServiceRps);
            lastMetaServiceCounter = currentMetaServiceCounter;
        }
