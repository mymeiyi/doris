...
class MetaServiceRpcRateLimiter {
    ...
    long acquire(String methodName, int permits) throws RpcException {
        ...
        int permitsToAcquire = Math.min(Math.max(permits, 1), holder.limitForPeriod);
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("meta service rpc rate limiter waits before acquiring permission, method: {}, permits: {}, "
                    ...
                    methodName, permitsToAcquire, holder.limitForPeriod, holder.burstSeconds,
                    waitMs);
        }
    ...
    private static class RateLimiterHolder {
        ...
        private RateLimiterHolder(RateLimiter rateLimiter, int limitForPeriod, int burstSeconds,
                long waitTimeoutMs) {
