...
suite("test_mow_compaction_and_cluster_key", "nonConcurrent") {
    ...
    def getLocalDeleteBitmapStatus = { tablet ->
        ...
        def code = process.waitFor()
        def out = process.getText()
        logger.info("Get local delete bitmap count status, code=" + code + ", out=" + out)
        assertEquals(code, 0)
        def deleteBitmapStatus = parseJson(out.trim())
        return deleteBitmapStatus
    }

    def wait_for_expected_layout = { def tablet, def stale_rowset_num, def dm_count, def dm_cardinality ->
        logger.info("wait for stale rowset num: " + stale_rowset_num)
        def tablet_status
        for (int i = 0; i < 100; i++) {
            tablet_status = getTabletStatus(tablet)
            if (tablet_status["stale_rowsets"].size() == stale_rowset_num) {
                break
            }
            sleep(100)
        }
        assertEquals(stale_rowset_num, tablet_status["stale_rowsets"].size())
        logger.info("wait for delete bitmap count: " + dm_count + ", cardinality" + dm_cardinality)
        def local_dm
        for (int i = 0; i < 20; i++) {
            local_dm = getLocalDeleteBitmapStatus(tablet)
            if (local_dm["delete_bitmap_count"] == dm_count && local_dm["cardinality"] == dm_cardinality) {
                break
            }
            sleep(2000)
        }
        assertEquals(dm_count, local_dm["delete_bitmap_count"])
        assertEquals(dm_cardinality, local_dm["cardinality"])
    }
    ...
    try {
        ...
        for (int method = 0; method < 2; method++) {
            ...
            def testTable = "test_mow_compaction_and_cluster_key_${method}"
            ...
            def partitions = sql_return_maparray """ show partitions from ${testTable}; """
            logger.info("partitions: " + partitions)
            assertEquals(1, partitions.size())
            def partition = partitions[0]
            def partitionId = partition.PartitionId
            ...
            // trigger compaction for [7-8]
            GetDebugPoint().enableDebugPointForAllBEs("SizeBasedCumulativeCompactionPolicy::pick_input_rowsets.set_input_rowsets",
                    [tablet_id: "${tablet.TabletId}", start_version: "${7}", end_version: "${8}"])
            getTabletStatus(tablet)
            assertTrue(triggerCompaction(tablet).contains("Success"))
            waitForCompaction(tablet)
            wait_for_expected_layout(tablet, 0, 1, 2)
            getTabletStatus(tablet)
            local_dm = getLocalDeleteBitmapStatus(tablet)
            logger.info(testTable + ", local_dm 1: " + local_dm)
            order_qt_sql3 "select * from ${testTable}"

            sql """ INSERT INTO ${testTable} VALUES (1, '101'); """
            // sql """ INSERT INTO ${testTable} VALUES (2, '101'); """
            sql """ INSERT INTO ${testTable} VALUES (3, '101'); """
            sql """ sync """
            order_qt_sql4 "select * from ${testTable}"

            GetDebugPoint().enableDebugPointForAllBEs("SizeBasedCumulativeCompactionPolicy::pick_input_rowsets.set_input_rowsets",
                    [tablet_id: "${tablet.TabletId}", start_version: "${9}", end_version: "${11}"])
            ...
            if (method == 0) {
                wait_for_expected_layout(tablet, 0, 3, 4)
            } else if (method == 1) {
                wait_for_expected_layout(tablet, 0, 2, 4)
            }
            ...
            logger.info("wait for no unused rowsets")*/
            getTabletStatus(tablet)
            local_dm = getLocalDeleteBitmapStatus(tablet)
            logger.info(testTable + ", local_dm 3: " + local_dm)
            if (method == 0) {
                // check is agged, wait for unused rowsets is deleted
                assertEquals(4, local_dm["cardinality"]) // the last one is agged
            } else if (method == 1) {
                assertEquals(2, local_dm["delete_bitmap_count"])
                assertEquals(4, local_dm["cardinality"])
            }

            GetDebugPoint().enableDebugPointForAllBEs("Tablet::_pick_visible_rowsets_to_compaction.set_visible_version",
                    [tablet_id: "${tablet.TabletId}", visible_version: "${8}"])
            // trigger compaction
            getTabletStatus(tablet)
            local_dm = getLocalDeleteBitmapStatus(tablet)
            logger.info(testTable + ", local_dm 4: " + local_dm)
            assertTrue(triggerBaseCompaction(tablet).contains("Success"))
            waitForCompaction(tablet)
            for (int i = 0; i < 100; i++) {
                def tablet_status = getTabletStatus(tablet)
            ...
            local_dm = getLocalDeleteBitmapStatus(tablet)
            logger.info(testTable + ", local_dm 5: " + local_dm)
            /*if (method == 0) {

            } else if (method == 1) {
                wait_for_expected_layout(tablet, 0, 2, 5)
            }*/
            // order_qt_sql6 "select * from ${testTable}"

            GetDebugPoint().clearDebugPointsForAllBEs()
            GetDebugPoint().enableDebugPointForAllFEs('OlapScanNode.addScanRangeLocations.set_partition_visible_version', [partitionId: partitionId, visibleVersion: 8])
            order_qt_sql6 "select * from ${testTable}"
