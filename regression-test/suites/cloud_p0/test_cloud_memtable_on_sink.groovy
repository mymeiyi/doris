...
suite("test_cloud_memtable_on_sink", "p0, docker") {
    ...
    docker(options) {
        ...
        def testDuplicate = {
            ...
            try {
                ...
                streamLoad {
                    ...
                    check { result, exception, startTime, endTime ->
                        ...
                        sql "SELECT assert_true('${response.Status}' = 'Fail')"
                    }
                }
            } finally {
            ...
            sql """
                SELECT assert_true(COUNT(*) = 20 AND SUM(k) = 1133 AND SUM(v) = 2266)
                FROM test_cloud_duplicate_memtable_on_sink_s3
            """
        ...
        // Unsupported options must still load correctly when forwarding is requested.
        sql "DROP TABLE IF EXISTS cloud_memtable_v1"
        sql """
            CREATE TABLE cloud_memtable_v1 (
                k INT NOT NULL, v INT, INDEX idx_v(v) USING INVERTED
            ) DUPLICATE KEY(k) DISTRIBUTED BY HASH(k) BUCKETS 1
            PROPERTIES ("replication_num"="1", "inverted_index_storage_format"="V1")
        """
        streamLoad {
            table "cloud_memtable_v1"
            set "column_separator", ","
            set "memtable_on_sink_node", "true"
            set "group_commit", "off_mode"
            inputText "1,10\n"
            check { result, exception, startTime, endTime ->
                if (exception != null) { throw exception }
                def response = parseJson(result)
                sql "SELECT assert_true('${response.Status}' = 'Success')"
            }
        }
        sql "SELECT assert_true(COUNT(*) = 1 AND SUM(v) = 10) FROM cloud_memtable_v1 WHERE k = 1"
    }
}
