...
suite("test_cloud_distributed_base_compaction", "docker") {
    ...
    docker(options) {
        ...
        def keyCases = [
            ...
            """],
            [name: "date", type: "DATE", keyExpr: """
                CAST(DAYS_ADD(CAST('2000-01-01' AS DATE), CAST(number AS INT)) AS DATE)
            """],
            [name: "datetime", type: "DATETIME", keyExpr: """
                CAST(SECONDS_ADD(CAST('2000-01-01 00:00:00' AS DATETIME),
                    CAST(number AS INT)) AS DATETIME)
            """],
