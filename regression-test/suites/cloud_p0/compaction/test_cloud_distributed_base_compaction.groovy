...
suite("test_cloud_distributed_base_compaction", "docker") {
    ...
    docker(options) {
        ...
        def keyCases = [
            ...
            """],
            [name: "boolean", keyColumns: "k BOOLEAN NULL, k2 INT NOT NULL",
             keyExpr: "IF(number % 3 = 0, CAST(NULL AS BOOLEAN), " +
                     "CAST(number % 2 AS BOOLEAN)), CAST(number AS INT)",
             keyModelColumns: "k, k2", sampleKey: "k, k2"],
            [name: "ipv4", keyColumns: "k IPV4 NULL, k2 INT NOT NULL", keyExpr: """
                IF(number < 4096, CAST(NULL AS IPV4),
                    TO_IPV4(IPV4_NUM_TO_STRING(number + 1))), CAST(number AS INT)
            """, keyModelColumns: "k, k2", sampleKey: "k, k2"],
            [name: "ipv6", keyColumns: "k IPV6 NULL, k2 INT NOT NULL", keyExpr: """
                IF(number < 4096, CAST(NULL AS IPV6),
                    CAST(CONCAT('2001:db8::', HEX(number + 1)) AS IPV6)), CAST(number AS INT)
            """, keyModelColumns: "k, k2", sampleKey: "k, k2"],
            [name: "timestamptz",
             keyColumns: "k TIMESTAMPTZ(6) NULL, k2 INT NOT NULL", keyExpr: """
                IF(number < 4096, CAST(NULL AS TIMESTAMPTZ(6)),
