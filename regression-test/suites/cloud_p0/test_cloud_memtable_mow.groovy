...
suite("test_cloud_memtable_mow", "p0, docker") {
    ...
    docker(options) {
        ...
        def logs = cluster.getAllBackends().collect { new File(it.getLogFilePath()) }
        ...
        try {
            awaitUntil(60) {
                logs.any { it.text.contains("start debug block " + block) }
            }
            sql "SET enable_memtable_on_sink_node=false"
        ...
        sql """
            ...
            PROPERTIES ("replication_num"="1", "enable_unique_key_merge_on_write"="true")
        """
        def label = "mow_direct_" + UUID.randomUUID().toString().replace('-', '_')
        sql "SET enable_profile=true"
        sql """
            LOAD LABEL ${label} (
        ...
        new ProfileAction(context).getProfileBySql(label,
                ["CloudMemtableDirectUpload: true", "CloudMemtableMowBitmap: true"])
        sql "SET enable_profile=false"
        order_qt_broker "SELECT * FROM cloud_mow_broker"
    }
}
