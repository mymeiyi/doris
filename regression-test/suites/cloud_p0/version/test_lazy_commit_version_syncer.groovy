...
suite("test_lazy_commit_version_syncer", "docker") {
    ...
    docker(options) {
        ...
        try {
            ...
            awaitUntil(20, 0.1) { lazyCommitCount("finished") > lazyBefore }
            ...
            // A waiting daemon can finish the task before the original RPC leaves its phase-one pause.
            // That RPC can then submit an additional no-op task for the already-visible transaction.
            // Check completion rather than requiring exactly one submission.
            awaitUntil(10, 0.1) { lazyCommitCount("submitted") == lazyCommitCount("finished") }
            // Lazy work finished before any retry and DELETE succeeded. No MS metadata keys or
            // transaction log text are inspected.
