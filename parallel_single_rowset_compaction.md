...
Worker endpoint 由 Coordinator 按需向 FE master 查询，不再使用静态列表。FE 根据
requester backend ID 反查其 Compute Group，只返回同组、alive、未退役且 BRPC 可用的
其他 BE。Coordinator 将结果缓存 `cloud_single_rowset_compaction_worker_cache_ttl_ms`，
并在 worker RPC 失败时立即失效缓存。一次 compaction 始终使用开始时取得的节点快照；
同一个 endpoint 上的 group 串行执行，不同 endpoint 并发执行。Worker 会同时校验
request 中的 target backend ID、`cloud_unique_id` 和 Compute Group ID，避免陈旧
endpoint 将任务发送到错误节点、其他 Compute Group 或其他 cloud instance。
