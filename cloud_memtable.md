...
本文对应当前分支（含 Cloud DUP、AGG、UNIQUE MOR/MOW 直传改动），静态核对日期为 2026-09-21。
...
| Sink BE 直传 | Sink BE 将 Segment / 索引直接上传对象存储，目标 BE 接收元数据并提交 Rowset；MOW 还传递 bitmap 结果 | SQL 会话变量、普通 Stream Load 请求头，或 HTTP Stream / Routine Load 的 FE 配置 |
...
| UNIQUE KEY，MOW | 支持全列写入，目标 BE 计算 bitmap | 支持全列写入 |
...
| `enable_cloud_memtable_sink_upload` | `true` | 普通 INSERT、Broker Load 的直传选择 |
...
| `cloud_stream_load_memtable_sink_upload` | `true` | HTTP Stream、Routine Load 以及 Group Commit 内部导入的直传开关；普通 Stream Load 使用请求头 |
...
ADMIN SET FRONTEND CONFIG ('cloud_stream_load_memtable_sink_upload' = 'true');
ADMIN SHOW FRONTEND CONFIG LIKE 'stream_load_default_%memtable%';
ADMIN SHOW FRONTEND CONFIG LIKE 'cloud_stream_load_memtable_sink_upload';
```
...
cloud_stream_load_memtable_sink_upload = true
```
...
直传使用请求头 `cloud_memtable_sink_upload`，未传时默认 `true`，不读取
`cloud_stream_load_memtable_sink_upload` 配置。在另一个 MySQL 连接执行
`SET enable_cloud_memtable_sink_upload=true` 对该 HTTP 请求无效。

发起带前移和直传参数的请求（CSV 文件内容例如 `1,10`、`2,20`）：
...
curl --location-trusted -u USER \
  ...
  -H 'cloud_memtable_sink_upload: true' \
...
显式 `memtable_on_sink_node:false` 可让该请求使用原写入路径。
保留前移但关闭直传：同时传入 `memtable_on_sink_node:true` 和 `cloud_memtable_sink_upload:false`。
普通 Stream Load 的 Profile 开关为 `enable_profile:true`，或 FE 的 `enable_stream_load_profile=true`。

### 3.4 HTTP Stream：`/api/_http_stream`

使用 SQL header 和 `http_stream(...)` 的入口，也接受 `memtable_on_sink_node`，直传读取 FE 配置 `cloud_stream_load_memtable_sink_upload`。
...
| 入口 | 使用方式 / 当前边界 |
|---|---|
| INSERT 或 Stream Load 的 Group Commit | 接收请求与最终内部导入是两个阶段。内部 `group_commit(...)` 导入没有携带单请求前移参数，读取 FE 的 `stream_load_default_memtable_on_sink_node` 和 `cloud_stream_load_memtable_sink_upload` 配置；不能只靠外层 INSERT 的 `SET` 或 HTTP header 判断最终 writer |
| Flink / Spark Connector / DataX / 自编写 HTTP 客户端，以 Stream Load 写入 | 通过客户端已有的 Stream Load header 透传能力发送 `memtable_on_sink_node:true`；或配置 FE 默认前移。普通 Stream Load 可透传 `cloud_memtable_sink_upload`，未传时默认 `true`；Group Commit 内部写入仍使用 FE 配置，不需要另加 S3 凭据到请求中 |
| MySQL `LOAD DATA` | MysqlLoadManager 转成 Stream Load，当前没有透传这两个 SQL 会话变量或前移 header；前移使用 FE 默认配置，普通 Stream Load 直传未传参数时默认 `true` |
...
各入口说明来自当前代码链路核对；已有测试覆盖见第 7 节，执行结果以对应版本的测试记录为准。
...
|---|---|---|---|---|
| 前移 + 目标 BE 上传 + packed | 开启 | 关闭 | `share_delta_writers=true`，`enable_packed_file=true` | 接收文件的目标 BE；DUP / AGG / MOR / MOW |
...
直传时，目标 BE 开启 `enable_file_cache` 且本次写入允许 `write_file_cache`，会在 Rowset
元数据提交成功后异步预热本机缓存，支持 packed slice 和独立文件，无需配置 event-driven warmup job。
预热遵循 `file_cache_enable_only_warm_up_idx` 等现有策略；导入返回不保证预热完成。

目标 BE 启用上述预热时，异机 Sink 禁止本次上传输出写入文件缓存，包括 adaptive cache admission；
MOW 读取旧数据的缓存行为不变。Sink 与目标位于同一 BE 时，保留上传时的缓存策略。
异机目标预热会增加对象存储读取流量，不改变事务可见性。
...
curl -u USER 'http://FE_HOST:8030/rest/v1/query_profile/text/PROFILE_ID'
```

详情接口返回 JSON，`data` 为未做 HTML 转换的 Profile 文本，解析后可直接搜索标记和计数器。
原 `/rest/v1/query_profile/PROFILE_ID` 接口仍可用，但会将换行和空格转换为 HTML。
...
当前分支已有回归用例覆盖普通 INSERT、Broker Load、普通 Stream Load；
覆盖 DUP / AGG / MOR / MOW、直传开关、packed、V2 索引和多 writer，MOW 还覆盖 Sequence、
删除/重插、cluster key、并发导入及 Compaction。
`test_cloud_memtable_group_commit` 覆盖 MOW 异步 Group Commit 的内部前移、Sink 上传及外层参数与
FE 配置的区别；`test_cloud_memtable_mow_forward` 覆盖 Group Commit 的 MOW 文件转发。
`test_cloud_memtable_sink_upload_cache` 覆盖目标 BE 预热、异机 Sink 输出不缓存及同机上传缓存。
其余入口在本文按代码传参链路说明，未新增专项测试。
以上为用例覆盖范围，不代表当前版本均已执行通过；执行结果以对应版本的测试记录为准。
本次仅静态核对并更新文档，未编译或运行测试。
...
| [test_cloud_memtable_sink_upload_mow.groovy](regression-test/suites/cloud_p0/test_cloud_memtable_sink_upload_mow.groovy) | MOW Profile、MS Rowset 布局、packed 开/关及重复 key 检查 |
