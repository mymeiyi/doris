...
struct DistributedCompactionTask {
    ...
    // Full-key encodings of this base-compaction task's [lower, upper) range.
    std::optional<std::string> encoded_lower_key = std::nullopt;
    std::optional<std::string> encoded_upper_key = std::nullopt;
    bool started = false;
};
