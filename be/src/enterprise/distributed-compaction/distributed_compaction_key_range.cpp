...
template <typename T>
unsigned __int128 estimate_max_key_range_weight(const std::vector<KeySample<T>>& samples,
                                                const std::vector<T>& boundaries) {
    std::vector<unsigned __int128> range_weights(boundaries.size() + 1);
    for (const auto& sample : samples) {
        const auto range = std::upper_bound(boundaries.begin(), boundaries.end(), sample.key);
        range_weights[range - boundaries.begin()] += sample.weight;
    }
    return *std::ranges::max_element(range_weights);
}
...
CompositeKeyRangePlan choose_composite_key_range_boundaries(
    ...
    unsigned __int128 best_max_range_weight = std::numeric_limits<unsigned __int128>::max();
    for (size_t prefix_length = 1; prefix_length <= key_columns; ++prefix_length) {
        ...
        auto boundaries = choose_key_range_boundaries(prefix_samples, range_count);
        const auto max_range_weight = estimate_max_key_range_weight(prefix_samples, boundaries);
        if (boundaries.size() > plan.boundaries.size() ||
            (!boundaries.empty() && boundaries.size() == plan.boundaries.size() &&
             max_range_weight < best_max_range_weight)) {
            plan = {.prefix_length = prefix_length, .boundaries = std::move(boundaries)};
            best_max_range_weight = max_range_weight;
        }
    }
    return plan;
}
...
Status select_boundaries_from_encoded_samples(
        ...
        size_t range_count, KeyRangeSamplingPlan* sampling_plan, KeyRangePlanningResult* result) {
...
// Locate the truncated short-key group containing each range target. Read that group and its next
// group so typed keys can refine boundaries within the group and at its upper edge.
...
Status read_refined_key_groups(const ShortKeyRefinementPlan& plan,
                               ...
                               KeyRangePlanningResult* result, RefinedKeyGroups* refined_groups) {
...
Status build_key_range_plan(const std::vector<RowsetSharedPtr>& input_rowsets,
    ...
    if (key_sampling_plan.uses_direct_encoded_boundaries()) {
        RETURN_IF_ERROR(select_boundaries_from_encoded_samples(
                std::move(encoded_key_samples), segment_key_samples, schema, range_count,
