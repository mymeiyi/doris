...
std::unique_ptr<ThreadPool> rpc_thread_pool;
...
Status start_distributed_compaction() {
    ...
    RETURN_IF_ERROR(ThreadPoolBuilder("CloudDistributedCompactionRpcThreadPool")
                            .set_min_threads(0)
                            .set_max_threads(config::cloud_distributed_compaction_rpc_thread_num)
                            .set_max_queue_size(config::cloud_distributed_compaction_rpc_queue_size)
                            .build(&rpc_thread_pool));
    RETURN_IF_ERROR(
            ThreadPoolBuilder("CloudDistributedCompactionWorkerThreadPool")
                    .set_min_threads(0)
                    .set_max_threads(config::cloud_distributed_compaction_worker_thread_num)
                    .set_max_queue_size(config::cloud_distributed_compaction_worker_queue_size)
                    .build(&worker_thread_pool));
    poll_scheduler = std::make_unique<DistributedCompactionPollScheduler>();
    return Status::OK();
}

void stop_distributed_compaction() {
    if (poll_scheduler != nullptr) {
        poll_scheduler->stop();
    }
}

void shutdown_distributed_compaction() {
    if (rpc_thread_pool != nullptr) {
        rpc_thread_pool->shutdown();
    }
    if (worker_thread_pool != nullptr) {
...
Status submit_distributed_compaction_worker_task(std::function<void()> task) {
    return distributed_compaction_worker_thread_pool().submit_func(std::move(task));
}
...
Status parse_worker_endpoint(std::string_view endpoint, std::string* host, int* port) {
...
Status call_worker(const std::string& endpoint, const Request& request, Response* response,
    ...
    std::string host;
    int port = 0;
    RETURN_IF_ERROR(parse_worker_endpoint(endpoint, &host, &port));
...
bool extract_boolean_value(const PValues& column, bool* value) {
    if (!column.has_type() || column.type().id() != PGenericType::BOOLEAN || column.has_null() ||
        !column.null_map().empty() || !column.double_value().empty() ||
        !column.float_value().empty() || !column.int32_value().empty() ||
        !column.int64_value().empty() || !column.uint32_value().empty() ||
        !column.uint64_value().empty() || column.bool_value_size() != 1 ||
        !column.string_value().empty() || !column.bytes_value().empty() ||
        !column.datetime_value().empty() || !column.child_element().empty() ||
        !column.child_offset().empty()) {
        return false;
    }
    *value = column.bool_value(0);
    return true;
}

bool is_ip_key(FieldType type) {
    return type == FieldType::OLAP_FIELD_TYPE_IPV4 || type == FieldType::OLAP_FIELD_TYPE_IPV6;
}
...
PGenericType_TypeId date_key_pb_type(FieldType type) {
    switch (type) {
...
FieldType date_key_field_type(PGenericType_TypeId type) {
    switch (type) {
...
PGenericType_TypeId ip_key_pb_type(FieldType type) {
    switch (type) {
...
FieldType ip_key_field_type(PGenericType_TypeId type) {
    switch (type) {
...
PGenericType_TypeId decimal_key_pb_type(FieldType type) {
    switch (type) {
...
FieldType decimal_key_field_type(PGenericType_TypeId type) {
    switch (type) {
...
PGenericType_TypeId key_pb_type(FieldType type) {
    if (type == FieldType::OLAP_FIELD_TYPE_BOOL) {
        return PGenericType::BOOLEAN;
    }
    if (is_integer_key(type)) {
        return integer_key_pb_type(type);
    }
    if (is_ip_key(type)) {
        return ip_key_pb_type(type);
    }
    if (is_date_key(type)) {
        return date_key_pb_type(type);
    }
    if (is_decimal_key(type)) {
        return decimal_key_pb_type(type);
    }
    DORIS_CHECK(is_string_key(type));
    return PGenericType::STRING;
}

bool is_supported_key_pb_type(PGenericType_TypeId type) {
    return type == PGenericType::BOOLEAN ||
           integer_key_field_type(type) != FieldType::OLAP_FIELD_TYPE_UNKNOWN ||
           ip_key_field_type(type) != FieldType::OLAP_FIELD_TYPE_UNKNOWN ||
           date_key_field_type(type) != FieldType::OLAP_FIELD_TYPE_UNKNOWN ||
           decimal_key_field_type(type) != FieldType::OLAP_FIELD_TYPE_UNKNOWN ||
           type == PGenericType::STRING;
}
...
bool is_null_key_field(const PValues& column) {
    return column.has_type() && is_supported_key_pb_type(column.type().id()) && column.has_null() &&
           column.null_map_size() == 1 && column.null_map(0) && column.double_value().empty() &&
           column.float_value().empty() && column.int32_value().empty() &&
           column.int64_value().empty() && column.uint32_value().empty() &&
           column.uint64_value().empty() && column.bool_value().empty() &&
           column.string_value().empty() && column.bytes_value().empty() &&
...
bool extract_date_key_field(const PValues& column, FieldType type, Field* field) {
    if (!column.has_type() || column.type().id() != date_key_pb_type(type) || column.has_null() ||
        !column.null_map().empty() || !column.double_value().empty() ||
        !column.float_value().empty() || !column.int32_value().empty() ||
        !column.bool_value().empty() || !column.string_value().empty() ||
        !column.bytes_value().empty() || !column.datetime_value().empty() ||
        !column.child_element().empty() || !column.child_offset().empty()) {
        return false;
    }
    switch (type) {
    case FieldType::OLAP_FIELD_TYPE_DATE:
    case FieldType::OLAP_FIELD_TYPE_DATETIME: {
        if (column.int64_value_size() != 1 || !column.uint32_value().empty() ||
            !column.uint64_value().empty()) {
            return false;
        }
        auto value = binary_cast<int64_t, VecDateTimeValue>(column.int64_value(0));
        const auto expected_type = type == FieldType::OLAP_FIELD_TYPE_DATE
                                           ? TimeType::TIME_DATE
                                           : TimeType::TIME_DATETIME;
        if (!value.is_valid_date() || value.type() != expected_type) {
            return false;
        }
        *field = type == FieldType::OLAP_FIELD_TYPE_DATE
                         ? Field::create_field<TYPE_DATE>(std::move(value))
                         : Field::create_field<TYPE_DATETIME>(std::move(value));
        return true;
    }
    case FieldType::OLAP_FIELD_TYPE_DATEV2: {
        if (!column.int64_value().empty() || column.uint32_value_size() != 1 ||
            !column.uint64_value().empty()) {
            return false;
        }
        DateV2Value<DateV2ValueType> value(column.uint32_value(0));
        if (!value.is_valid_date()) {
            return false;
        }
        *field = Field::create_field<TYPE_DATEV2>(std::move(value));
        return true;
    }
    case FieldType::OLAP_FIELD_TYPE_DATETIMEV2: {
        if (!column.int64_value().empty() || !column.uint32_value().empty() ||
            column.uint64_value_size() != 1) {
            return false;
        }
        DateV2Value<DateTimeV2ValueType> value(column.uint64_value(0));
        if (!value.is_valid_date()) {
            return false;
        }
        *field = Field::create_field<TYPE_DATETIMEV2>(std::move(value));
        return true;
    }
    case FieldType::OLAP_FIELD_TYPE_TIMESTAMPTZ: {
        if (!column.int64_value().empty() || !column.uint32_value().empty() ||
        ...
        TimestampTzValue value(column.uint64_value(0));
        if (!value.is_valid_date()) {
            return false;
        }
        *field = Field::create_field<TYPE_TIMESTAMPTZ>(std::move(value));
        return true;
    }
    default:
        DORIS_CHECK(false) << "unsupported distributed base compaction key type: " << int(type);
...
bool extract_ip_key_field(const PValues& column, FieldType type, Field* field) {
    if (!column.has_type() || column.type().id() != ip_key_pb_type(type) || column.has_null() ||
        !column.null_map().empty() || !column.double_value().empty() ||
        !column.float_value().empty() || !column.int32_value().empty() ||
        !column.int64_value().empty() || !column.uint64_value().empty() ||
        !column.bool_value().empty() || !column.string_value().empty() ||
        !column.datetime_value().empty() || !column.child_element().empty() ||
        !column.child_offset().empty()) {
        return false;
    ...
    switch (type) {
    case FieldType::OLAP_FIELD_TYPE_IPV4:
        if (column.uint32_value_size() != 1 || !column.bytes_value().empty()) {
            return false;
        }
        *field = Field::create_field<TYPE_IPV4>(column.uint32_value(0));
        return true;
    case FieldType::OLAP_FIELD_TYPE_IPV6: {
        if (!column.uint32_value().empty() || column.bytes_value_size() != 1 ||
            column.bytes_value(0).size() != sizeof(IPv6)) {
            return false;
        }
        IPv6 value;
        memcpy(&value, column.bytes_value(0).data(), sizeof(value));
        *field = Field::create_field<TYPE_IPV6>(value);
        return true;
    }
    default:
        DORIS_CHECK(false) << "unsupported distributed base compaction key type: " << int(type);
...
template <PrimitiveType primitive_type, typename NativeType>
bool extract_decimal_key_value(const PValues& column, Field* field) {
    if (column.bytes_value_size() != 1 || column.bytes_value(0).size() != sizeof(NativeType)) {
        return false;
    }
    NativeType value;
    memcpy(&value, column.bytes_value(0).data(), sizeof(value));
    using DecimalType = typename PrimitiveTypeTraits<primitive_type>::CppType;
    *field = Field::create_field<primitive_type>(DecimalType(value));
    return true;
}

bool extract_decimal_key_field(const PValues& column, FieldType type, Field* field) {
    if (!column.has_type() || column.type().id() != decimal_key_pb_type(type) ||
        column.has_null() || !column.null_map().empty() || !column.double_value().empty() ||
        !column.float_value().empty() || !column.int32_value().empty() ||
        !column.int64_value().empty() || !column.uint32_value().empty() ||
        !column.uint64_value().empty() || !column.bool_value().empty() ||
        !column.string_value().empty() || !column.datetime_value().empty() ||
        !column.child_element().empty() || !column.child_offset().empty()) {
        return false;
    }
    switch (type) {
    case FieldType::OLAP_FIELD_TYPE_DECIMAL:
        return extract_decimal_key_value<TYPE_DECIMALV2, int128_t>(column, field);
    case FieldType::OLAP_FIELD_TYPE_DECIMAL32:
        return extract_decimal_key_value<TYPE_DECIMAL32, int32_t>(column, field);
    case FieldType::OLAP_FIELD_TYPE_DECIMAL64:
        return extract_decimal_key_value<TYPE_DECIMAL64, int64_t>(column, field);
    case FieldType::OLAP_FIELD_TYPE_DECIMAL128I:
        return extract_decimal_key_value<TYPE_DECIMAL128I, int128_t>(column, field);
    case FieldType::OLAP_FIELD_TYPE_DECIMAL256:
        return extract_decimal_key_value<TYPE_DECIMAL256, wide::Int256>(column, field);
    default:
        DORIS_CHECK(false) << "unsupported distributed base compaction key type: " << int(type);
        __builtin_unreachable();
    }
...
bool extract_key_fields(const PCloudDistributedCompactionKey& key,
    ...
    for (const auto& column : key.columns()) {
        ...
        if (type == PGenericType::BOOLEAN) {
            bool value = false;
            if (!extract_boolean_value(column, &value)) {
                return false;
            }
            fields->push_back(Field::create_field<TYPE_BOOLEAN>(value));
        } else if (const FieldType field_type = integer_key_field_type(type);
                   field_type != FieldType::OLAP_FIELD_TYPE_UNKNOWN) {
        ...
        } else if (const FieldType ip_type = ip_key_field_type(type);
                   ip_type != FieldType::OLAP_FIELD_TYPE_UNKNOWN) {
            Field field;
            if (!extract_ip_key_field(column, ip_type, &field)) {
                return false;
            }
            fields->push_back(std::move(field));
        } else if (const FieldType date_type = date_key_field_type(type);
                   date_type != FieldType::OLAP_FIELD_TYPE_UNKNOWN) {
            Field field;
            if (!extract_date_key_field(column, date_type, &field)) {
                return false;
            }
            fields->push_back(std::move(field));
        } else if (const FieldType decimal_type = decimal_key_field_type(type);
                   decimal_type != FieldType::OLAP_FIELD_TYPE_UNKNOWN) {
            Field field;
            if (!extract_decimal_key_field(column, decimal_type, &field)) {
                return false;
            }
            fields->push_back(std::move(field));
...
void set_boolean_key_column(PValues* column, const Field& field) {
    column->mutable_type()->set_id(PGenericType::BOOLEAN);
    column->add_bool_value(field.get<TYPE_BOOLEAN>() != 0);
}

void set_ip_key_column(PValues* column, FieldType type, const Field& field) {
    column->mutable_type()->set_id(ip_key_pb_type(type));
    switch (type) {
    case FieldType::OLAP_FIELD_TYPE_IPV4:
        column->add_uint32_value(field.get<TYPE_IPV4>());
        return;
    case FieldType::OLAP_FIELD_TYPE_IPV6: {
        const auto& value = field.get<TYPE_IPV6>();
        column->add_bytes_value(&value, sizeof(value));
        return;
    }
    default:
...
void set_date_key_column(PValues* column, FieldType type, const Field& field) {
    column->mutable_type()->set_id(date_key_pb_type(type));
    switch (type) {
    case FieldType::OLAP_FIELD_TYPE_DATE:
        column->add_int64_value(binary_cast<VecDateTimeValue, int64_t>(field.get<TYPE_DATE>()));
        return;
    case FieldType::OLAP_FIELD_TYPE_DATETIME:
        column->add_int64_value(binary_cast<VecDateTimeValue, int64_t>(field.get<TYPE_DATETIME>()));
        return;
    case FieldType::OLAP_FIELD_TYPE_DATEV2:
        column->add_uint32_value(field.get<TYPE_DATEV2>().to_date_int_val());
        return;
    case FieldType::OLAP_FIELD_TYPE_DATETIMEV2:
        column->add_uint64_value(field.get<TYPE_DATETIMEV2>().to_date_int_val());
        return;
    case FieldType::OLAP_FIELD_TYPE_TIMESTAMPTZ:
        column->add_uint64_value(field.get<TYPE_TIMESTAMPTZ>().to_date_int_val());
        return;
    default:
        DORIS_CHECK(false) << "unsupported distributed base compaction key type: " << int(type);
        __builtin_unreachable();
...
void set_decimal_key_value(PValues* column, const Field& field) {
    const auto& value = field.get<primitive_type>();
    column->add_bytes_value(&value.value, sizeof(value.value));
}

void set_decimal_key_column(PValues* column, FieldType type, const Field& field) {
    column->mutable_type()->set_id(decimal_key_pb_type(type));
    switch (type) {
    case FieldType::OLAP_FIELD_TYPE_DECIMAL: {
        const auto& value = field.get<TYPE_DECIMALV2>().value();
        column->add_bytes_value(&value, sizeof(value));
        return;
    }
    case FieldType::OLAP_FIELD_TYPE_DECIMAL32:
        set_decimal_key_value<TYPE_DECIMAL32>(column, field);
        return;
    case FieldType::OLAP_FIELD_TYPE_DECIMAL64:
        set_decimal_key_value<TYPE_DECIMAL64>(column, field);
        return;
    case FieldType::OLAP_FIELD_TYPE_DECIMAL128I:
        set_decimal_key_value<TYPE_DECIMAL128I>(column, field);
        return;
    case FieldType::OLAP_FIELD_TYPE_DECIMAL256:
        set_decimal_key_value<TYPE_DECIMAL256>(column, field);
        return;
    default:
        DORIS_CHECK(false) << "unsupported distributed base compaction key type: " << int(type);
        __builtin_unreachable();
...
void set_null_key_column(PValues* column, FieldType type) {
    column->mutable_type()->set_id(key_pb_type(type));
    column->set_has_null(true);
    column->add_null_map(true);
}
...
void set_key_fields(PCloudDistributedCompactionKey* key, const TabletSchema& schema,
    ...
    for (size_t column_index = 0; column_index < fields.size(); ++column_index) {
        ...
        if (fields[column_index].is_null()) {
            DORIS_CHECK(schema.column(column_index).is_nullable());
            set_null_key_column(column, type);
            continue;
        }
        if (type == FieldType::OLAP_FIELD_TYPE_BOOL) {
            set_boolean_key_column(column, fields[column_index]);
        } else if (is_integer_key(type)) {
            set_integer_key_column(column, type, get_integer_key_field(fields[column_index], type));
        } else if (is_ip_key(type)) {
            set_ip_key_column(column, type, fields[column_index]);
        } else if (is_date_key(type)) {
            set_date_key_column(column, type, fields[column_index]);
        } else if (is_decimal_key(type)) {
            set_decimal_key_column(column, type, fields[column_index]);
        } else {
            DORIS_CHECK(is_string_key(type));
...
Status encode_key_range_boundary(const TabletSchemaSPtr& schema, const CompositeKey& fields,
                                 std::string* encoded_key) {
    RowCursor cursor;
    RETURN_IF_ERROR(cursor.init_scan_key(schema, fields));
    encoded_key->clear();
    cursor.encode_key_with_padding<true>(encoded_key, schema->num_key_columns(), true);
    return Status::OK();
}
...
bool extract_key_tuple(const PCloudDistributedCompactionKey& key, const TabletSchema& schema,
    ...
    for (size_t column_index = 0; column_index < fields.size(); ++column_index) {
        ...
        if (!is_supported_key_range_column_type(column.type()) ||
            (fields[column_index].is_null() && !column.is_nullable())) {
            return false;
        }
        if (types[column_index] != key_pb_type(column.type())) {
            return false;
        }
...
Status validate_distributed_compaction_task(const PCloudDistributedCompactionSubmitRequest& request,
    ...
    const bool valid_key_range =
            ...
            (!key_range.has_lower_key() || !key_range.has_upper_key() ||
...
Status DistributedCompactionPollScheduler::schedule(std::function<void()> callback) {
    DORIS_CHECK(callback != nullptr);
    DORIS_CHECK_GT(config::cloud_distributed_compaction_status_poll_interval_ms, 0);
    const auto deadline =
            std::chrono::steady_clock::now() +
            std::chrono::milliseconds(config::cloud_distributed_compaction_status_poll_interval_ms);
    {
        std::lock_guard lock(_mutex);
        if (_stopped) {
            return Status::Cancelled("distributed compaction poll scheduler is stopped");
        }
        _callbacks.emplace(std::make_pair(deadline, _next_sequence++), std::move(callback));
    }
    _cv.notify_one();
    return Status::OK();
}
...
int64_t DistributedCompactionCoordinator::task_count() const {
    DORIS_CHECK(_state != nullptr);
    return cast_set<int64_t>(_state->tasks.size());
}

int64_t DistributedCompactionCoordinator::worker_count() const {
    DORIS_CHECK(_execution_plan != nullptr);
    return cast_set<int64_t>(_execution_plan->workers.size());
}
...
Status DistributedCompactionCoordinator::start_base_compaction(
        ...
        bool is_vertical, uint32_t avg_segment_rows, CompletionCallback callback, bool* started) {
    DORIS_CHECK(callback != nullptr);
    const int64_t target_size =
            config::cloud_distributed_base_compaction_target_range_input_size_bytes;
    DORIS_CHECK_GT(target_size, 0);
    int64_t input_rowsets_total_size = 0;
    for (const auto& rowset : input_rowsets) {
        input_rowsets_total_size += rowset->total_disk_size();
    }
    const size_t range_count = cast_set<size_t>(1 + (input_rowsets_total_size - 1) / target_size);
    RETURN_IF_ERROR(try_submit_base_compaction_tasks(input_rowsets, output_rowset_writer,
                                                     range_count, is_vertical, avg_segment_rows,
                                                     started));
    if (!*started) {
        return Status::OK();
    }
    _execution_plan->input_rowsets_total_size = input_rowsets_total_size;
    _execution_plan->completion_callback = std::move(callback);
    return schedule_poll();
}

Status DistributedCompactionCoordinator::schedule_poll() {
    DORIS_CHECK(_execution_plan != nullptr);
    DORIS_CHECK(!_execution_plan->polling_completed);
    auto weak_self = weak_from_this();
    return schedule_distributed_compaction([weak_self] {
...
void DistributedCompactionCoordinator::dispatch_poll() {
    ...
    for (size_t worker_index = 0; worker_index < _execution_plan->workers.size(); ++worker_index) {
        ...
        for (const size_t group_index : _execution_plan->groups_by_worker[worker_index]) {
            ...
            status_request.add_group_indexes(distributed_task.group_index);
        }
    ...
    for (size_t worker_index = 0; worker_index < _execution_plan->workers.size(); ++worker_index) {
        ...
        const Status submit_status =
                distributed_compaction_rpc_thread_pool().submit_func([self, round, worker_index] {
...
void DistributedCompactionCoordinator::finish_poll_round(std::shared_ptr<PollRoundContext> round) {
    ...
    const Status schedule_status = schedule_poll();
...
Status validate_distributed_compaction_segment_key_bounds(
    ...
    for (size_t segment_index = 0; segment_index < key_bounds.size(); ++segment_index) {
        ...
        if (Slice(current.max_key()).compare(Slice(current.min_key())) < 0) {
            return Status::InvalidArgument("segment {} has an invalid key range", segment_index);
        }
        if (segment_index == 0) {
            continue;
        }
        ...
        } else if (Slice::lhs_is_strictly_less_than_rhs(Slice(current.min_key()), true,
                                                        Slice(previous.max_key()), true)) {
            // Equal truncated prefixes are inconclusive and remain valid for rolling upgrades.
            return Status::InvalidArgument("segments {} and {} have reversed key ranges",
                                           segment_index - 1, segment_index);
        }
    }
    return Status::OK();
...
Status validate_distributed_compaction_task_key_bounds(size_t group_index,
                                                       const std::vector<KeyBoundsPB>& key_bounds,
                                                       bool key_bounds_truncated,
                                                       const DistributedCompactionTask& task) {
    if (key_bounds.empty()) {
        return Status::OK();
    }
    if (task.encoded_lower_key.has_value() &&
        Slice::lhs_is_strictly_less_than_rhs(Slice(key_bounds.front().min_key()),
                                             key_bounds_truncated, Slice(*task.encoded_lower_key),
                                             false)) {
        return Status::InvalidArgument("group {} output is below its lower key bound", group_index);
    }
    if (task.encoded_upper_key.has_value()) {
        const Slice output_max(key_bounds.back().max_key());
        const Slice upper_key(*task.encoded_upper_key);
        if (output_max.compare(upper_key) >= 0) {
            return Status::InvalidArgument("group {} output reaches or exceeds its upper key bound",
                                           group_index);
        }
    }
    return Status::OK();
...
Status validate_distributed_compaction_partial_rowset(
    ...
    for (const int64_t segment_id : partial_meta_pb.segment_ids()) {
        ...
        if (segment_id < task.segment_id_slot.start_id ||
            segment_id >= cast_set<int64_t>(task.segment_id_slot.start_id) +
                                  task.segment_id_slot.capacity) {
            return Status::InvalidArgument(
...
Status DistributedCompactionCoordinator::validate_partial_rowset(
    ...
    return validate_distributed_compaction_partial_rowset(
            group_index, response, task, tablet_schema, output_rowset_id, &partial_rowset->meta,
            &partial_rowset->key_bounds, &partial_rowset->segment_rows);
}
...
Status DistributedCompactionCoordinator::prepare_single_rowset(
    ...
    *started = false;
    ...
    for (size_t worker_index = 0; worker_index < workers.size(); ++worker_index) {
        ...
        *request.add_input_rowset_metas() = input_meta_pb;
        *request.mutable_output_rowset_meta() = output_meta_pb;
        ...
        for (const size_t group_index : groups_by_worker[worker_index]) {
            ...
            request_task->set_segment_pos_start(range.segment_start);
            request_task->set_segment_pos_end(range.segment_end);
...
Status DistributedCompactionCoordinator::try_submit_base_compaction_tasks(
        const std::vector<RowsetSharedPtr>& input_rowsets, RowsetWriter& output_rowset_writer,
    ...
    *started = false;
    ...
    if (is_mow) {
        ...
        if (check_missed_rows) {
            ...
            source_delete_rows = cast_set<int64_t>(aggregated_source_delete_bitmap.cardinality());
        }
    }
    _state->tasks.reserve(group_count);
    std::vector<std::string> encoded_range_boundaries;
    encoded_range_boundaries.reserve(key_range_plan.boundaries.size());
    for (const auto& boundary : key_range_plan.boundaries) {
        encoded_range_boundaries.emplace_back();
        RETURN_IF_ERROR(encode_key_range_boundary(_tablet->tablet_schema(), boundary,
                                                  &encoded_range_boundaries.back()));
    }
    auto groups_by_worker = assign_compaction_groups_round_robin(group_count, workers.size());
    for (size_t group_index = 0; group_index < group_count; ++group_index) {
        const size_t worker_index = group_index % workers.size();
        DistributedCompactionTask task {.worker_endpoint = workers[worker_index].endpoint,
                                        .group_index = cast_set<int32_t>(group_index),
                                        .segment_id_slot = segment_slots[group_index]};
        if (group_index > 0) {
            task.encoded_lower_key = encoded_range_boundaries[group_index - 1];
        }
        if (group_index + 1 < group_count) {
            task.encoded_upper_key = encoded_range_boundaries[group_index];
        }
    ...
    LOG_INFO("finish distributed base compaction range planning")
            ...
            .tag("primary_key_index_load_time_us",
                 key_range_planning.primary_key_index_load_time_us)
            .tag("primary_key_sample_read_time_us",
                 key_range_planning.primary_key_sample_read_time_us)
            .tag("key_sample_read_time_us", key_range_planning.key_sample_read_time_us)
            .tag("boundary_key_read_time_us", key_range_planning.boundary_key_read_time_us)
            .tag("local_read_bytes", key_range_planning.planning_io_stats.bytes_read_from_local)
            .tag("remote_read_bytes", key_range_planning.planning_io_stats.bytes_read_from_remote)
            .tag("peer_read_bytes", key_range_planning.planning_io_stats.bytes_read_from_peer)
            .tag("local_read_count", key_range_planning.planning_io_stats.num_local_io_total)
            .tag("remote_read_count", key_range_planning.planning_io_stats.num_remote_io_total)
            .tag("peer_read_count", key_range_planning.planning_io_stats.num_peer_io_total)
            .tag("local_read_time_us", key_range_planning.planning_io_stats.local_io_timer / 1000)
            .tag("remote_read_time_us", key_range_planning.planning_io_stats.remote_io_timer / 1000)
            .tag("peer_read_time_us", key_range_planning.planning_io_stats.peer_io_timer / 1000)
            .tag("primary_key_sample_local_read_bytes",
                 key_range_planning.primary_key_sample_io_stats.bytes_read_from_local)
            .tag("primary_key_sample_remote_read_bytes",
                 key_range_planning.primary_key_sample_io_stats.bytes_read_from_remote)
            .tag("primary_key_sample_peer_read_bytes",
                 key_range_planning.primary_key_sample_io_stats.bytes_read_from_peer)
            .tag("primary_key_sample_local_read_count",
                 key_range_planning.primary_key_sample_io_stats.num_local_io_total)
            .tag("primary_key_sample_remote_read_count",
                 key_range_planning.primary_key_sample_io_stats.num_remote_io_total)
            .tag("primary_key_sample_peer_read_count",
                 key_range_planning.primary_key_sample_io_stats.num_peer_io_total)
            .tag("primary_key_sample_local_read_time_us",
                 key_range_planning.primary_key_sample_io_stats.local_io_timer / 1000)
            .tag("primary_key_sample_remote_read_time_us",
                 key_range_planning.primary_key_sample_io_stats.remote_io_timer / 1000)
            .tag("primary_key_sample_peer_read_time_us",
                 key_range_planning.primary_key_sample_io_stats.peer_io_timer / 1000)
            .tag("key_sample_local_read_bytes",
                 key_range_planning.key_sample_io_stats.bytes_read_from_local)
            .tag("key_sample_remote_read_bytes",
                 key_range_planning.key_sample_io_stats.bytes_read_from_remote)
            .tag("key_sample_peer_read_bytes",
                 key_range_planning.key_sample_io_stats.bytes_read_from_peer)
            .tag("key_sample_local_read_count",
                 key_range_planning.key_sample_io_stats.num_local_io_total)
            .tag("key_sample_remote_read_count",
                 key_range_planning.key_sample_io_stats.num_remote_io_total)
            .tag("key_sample_peer_read_count",
                 key_range_planning.key_sample_io_stats.num_peer_io_total)
            .tag("boundary_choose_time_us", key_range_planning.boundary_choose_time_us)
...
Status DistributedCompactionCoordinator::fetch_incremental_delete_bitmap(
    ...
    for (size_t worker_index = 0; worker_index < worker_endpoints.size(); ++worker_index) {
        ...
        for (const size_t task_index : groups_by_worker[worker_index]) {
            ...
            request.add_group_indexes(task.group_index);
        }
    }
...
void DistributedCompactionCoordinator::finalize(bool cancel_tasks) {
    if (_state == nullptr) {
        return;
    }
    ...
    if (!groups_by_worker.empty()) {
        const auto finalize_endpoint = [&](const std::string& endpoint,
            ...
            request.set_execution_id(_execution_id);
            request.set_cancel_tasks(cancel_tasks);
...
void DistributedCompactionWorker::cancel_compaction(const Status& status) {
...
Status DistributedCompactionWorker::handle_compaction(
    ...
    if (is_base) {
        ...
        if (!supported_keys_type || schema.num_key_columns() == 0 ||
            !is_supported_key_range_column_type(schema.column(0).type())) {
            return Status::InvalidArgument(
                    "distributed base compaction requires DUP_KEYS, AGG_KEYS, UNIQUE_KEYS MOR, "
                    "or non-cluster-key UNIQUE_KEYS MOW tablet with a supported leading key");
        }
    }
    std::vector<RowsetSharedPtr> input_rowsets;
    input_rowsets.reserve(cast_set<size_t>(request->input_rowset_metas_size()));
    for (const auto& input_meta_pb : request->input_rowset_metas()) {
        ...
        if (input_meta->tablet_id() != _tablet->tablet_id()) {
        ...
        RETURN_IF_ERROR(RowsetFactory::create_rowset(
                _tablet->tablet_schema(), _tablet->tablet_path(), input_meta, &input_rowset));
    ...
    if (is_base) {
        ...
        const auto& schema = *_tablet->tablet_schema();
        if (key_range.has_lower_key()) {
            lower_key.emplace();
            if (!extract_key_tuple(key_range.lower_key(), schema, &*lower_key)) {
                return Status::InvalidArgument(
                        "distributed base compaction lower key does not match tablet key type");
            }
        }
        if (key_range.has_upper_key()) {
            upper_key.emplace();
            if (!extract_key_tuple(key_range.upper_key(), schema, &*upper_key)) {
                return Status::InvalidArgument(
                        "distributed base compaction upper key does not match tablet key type");
            }
        }
    ...
    if (output_meta.tablet_id() != _tablet->tablet_id()) {
    ...
    if (_is_mow) {
        const auto mode =
                is_base ? RowIdConversion::Mode::LAZY_CHUNKED : RowIdConversion::Mode::DENSE;
    ...
    for (const auto& input_rowset : input_rowsets) {
        ...
        if (!request->is_coordinator() && request->has_coordinator_host()) {
            reader->set_preferred_file_cache_peer(request->coordinator_host(),
...
Status DistributedCompactionWorkerManager::submit(
    ...
    if (tablet->keys_type() == KeysType::UNIQUE_KEYS &&
        ...
        for (const auto& input_meta : request.input_rowset_metas()) {
            const auto synced_rowset = tablet->get_rowset_by_version(
                    {input_meta.start_version(), input_meta.end_version()});
            if (synced_rowset == nullptr ||
                synced_rowset->rowset_id().to_string() != input_meta.rowset_id_v2()) {
                return Status::Aborted(
                        "distributed compaction input rowset is stale: version=[{}-{}], "
                        "expected_rowset={}",
                        input_meta.start_version(), input_meta.end_version(),
                        input_meta.rowset_id_v2());
            }
        }
    }
    ...
    Status submit_status = Status::OK();
    {
        ...
        for (const auto& task : request.tasks()) {
            ...
            auto worker =
                    std::make_shared<DistributedCompactionWorker>(engine, tablet, arrival_time_us);
            _workers.emplace(
                    worker_key,
        ...
        auto* worker_pool = &distributed_compaction_worker_thread_pool();
        submit_status = worker_pool->submit_func([request_copy, batch, worker_pool]() {
            for (size_t job_index = 0; job_index < batch->jobs.size(); ++job_index) {
                const Status task_submit_status = worker_pool->submit_func([request_copy, batch,
                                                                            job_index]() {
                    const auto& job = batch->jobs[job_index];
                    Status task_status = Status::OK();
                    if (request_copy->output_rowset_meta().txn_expiration() <= ::time(nullptr)) {
                        task_status = Status::TimedOut(
                                "distributed compaction task expired before execution");
                        job.worker->cancel_compaction(task_status);
                    } else {
                        auto task_request = *request_copy;
                        task_status = job.worker->execute_compaction(&task_request, &job.task);
                    }
                    if (!task_status.ok()) {
                ...
                if (!task_submit_status.ok()) {
                    batch->cancel(
                            Status::TooManyTasks("failed to submit distributed compaction task: {}",
        ...
        if (!submit_status.ok()) {
            ...
            for (const auto& created_worker : created_workers) {
                ...
                iter->second.worker->cancel_compaction(cancel_status);
...
Status DistributedCompactionWorkerManager::calc_incremental_delete_bitmap(
        const PCloudDistributedCompactionCalcIncrementalDeleteBitmapRequest& request,
        PCloudDistributedCompactionCalcIncrementalDeleteBitmapResponse* response) {
    if (request.tablet_id() <= 0 || request.execution_id().empty() ||
        request.group_indexes().empty() ||
    ...
    std::unordered_set<std::string> request_keys;
    request_keys.reserve(cast_set<size_t>(request.group_indexes_size()));
    ...
    for (const int32_t group_index : request.group_indexes()) {
        ...
        const std::string worker_key = key(request.execution_id(), group_index);
        if (!request_keys.emplace(worker_key).second) {
            return Status::InvalidArgument(
        ...
        if (worker->_tablet->tablet_id() != request.tablet_id()) {
            return Status::InvalidArgument(
                    ...
                    request.execution_id(), group_index, request.tablet_id(),
...
bool can_use_distributed_base_compaction(const CloudTablet& tablet,
    ...
    const int64_t target_size =
            config::cloud_distributed_base_compaction_target_range_input_size_bytes;
    if (target_size <= 0 || input_rowsets_total_size <= target_size ||
        tablet.is_row_binlog_tablet()) {
        return false;
    }

    const auto& schema = *tablet.tablet_schema();
    if (schema.sort_type() == SortType::ZORDER || schema.num_key_columns() == 0 ||
        !is_supported_key_range_column_type(schema.column(0).type())) {
        return false;
    }
...
Status start_distributed_base_compaction(
    ...
    auto coordinator = std::make_shared<DistributedCompactionCoordinator>(engine, std::move(tablet),
                                                                          std::move(execution_id));
    bool started = false;
    RETURN_IF_ERROR(coordinator->start_base_compaction(input_rowsets, output_rowset_writer,
                                                       is_vertical, avg_segment_rows,
                                                       std::move(completion), &started));
    if (started) {
        *compaction = std::move(coordinator);
    } else {
        compaction->reset();
    }
    return Status::OK();
...
Status start_distributed_single_rowset_compaction(
    ...
    auto coordinator = std::make_shared<DistributedCompactionCoordinator>(engine, std::move(tablet),
                                                                          std::move(execution_id));
    bool started = false;
    RETURN_IF_ERROR(coordinator->start_single_rowset_compaction(
            input_rowset, output_rowset_writer, segment_group_size, allow_delete_in_cumu_compaction,
            is_vertical, avg_segment_rows, std::move(completion), &started));
    if (started) {
        *compaction = std::move(coordinator);
    } else {
        compaction->reset();
...
void remove_expired_distributed_compactions(int64_t current_time) {
    DistributedCompactionWorkerManager::instance()->remove_expired_workers(current_time);
}
