...
Status LoadBlockQueue::get_block(
        RuntimeState* runtime_state, vectorized::Block* block, bool* find_block, bool* eos,
        std::shared_ptr<pipeline::Dependency> get_block_dep,
        std::shared_ptr<pipeline::Dependency> runtime_filter_timer_dependency) {
    *find_block = false;
    *eos = false;
    std::unique_lock l(mutex);
    if (runtime_state->is_cancelled() || !status.ok()) {
    ...
    if (_block_queue.empty()) {
        ...
        if (!_need_commit && !runtime_filter_timer_dependency->ready()) {
            get_block_dep->block();
