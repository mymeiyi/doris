...
Status GroupCommitOperatorX::get_block(RuntimeState* state, vectorized::Block* block, bool* eos) {
    ...
    RETURN_IF_ERROR(local_state.load_block_queue->get_block(
            state, block, &find_node, eos, local_state._get_block_dependency,
            local_state._runtime_filter_timer_dependency));
    return Status::OK();
}
...
Status GroupCommitLocalState::init(RuntimeState* state, LocalStateInfo& info) {
    ...
    _runtime_filter_timer_dependency = Dependency::create_shared(
            _parent->operator_id(), _parent->node_id(), "GroupCommitRTFDependency", true);
    ...
    if (st.ok()) {
        ...
        _runtime_filter_timer_dependency->block();
        _runtime_filter_timer = std::make_shared<pipeline::RuntimeFilterTimer>(
                ...
                _runtime_filter_timer_dependency);
