...
Status CloudBaseCompaction::execute_compact() {
    // Distributed Base is scheduled through execute_distributed_compact_async(); this synchronous
    // path only runs local Base compaction.
#ifndef __APPLE__
    ...
    record_compaction_success(execution_start_time_us);
    return Status::OK();
}

void CloudBaseCompaction::record_compaction_success(int64_t execution_start_time_us) {
