    ...
    *distributed_started = false;
    ...
    try {
        ...
        status = _distributed_compaction->start_single_rowset_compaction(
                ...
                std::move(remote_completion), &started);
        if (started) {
        ...
        status = do_local_single_rowset_grouped_compaction(&_merge_execution_context->result);
        if (!status.ok()) {
            return handle_prepared_compaction_failure(std::move(status));
        }
...
Status CloudCumulativeCompaction::complete_prepared_compaction() {
    DORIS_CHECK(_merge_execution_context != nullptr);
