...
bool should_use_single_rowset_grouped_compaction(
        const std::vector<RowsetSharedPtr>& input_rowsets, const TabletSchema& tablet_schema) {
    return tablet_schema.num_key_columns() > 0 && config::enable_cloud_single_rowset_compaction &&
    ...
    if (config::enable_cloud_single_rowset_compaction) {
        for (const auto& rowset : candidate_rowsets) {
            if (cloud::should_use_single_rowset_grouped_compaction(
                        {rowset}, *cloud_tablet()->tablet_schema())) {
                _input_rowsets = {rowset};
                return Status::OK();
            }
        }
    }
...
Status CloudCumulativeCompaction::prepare_merge_input_rowsets(MergeInputRowsetsResult* result) {
    if (!cloud::should_use_single_rowset_grouped_compaction(_input_rowsets, *_cur_tablet_schema)) {
        return Status::OK();
    }
