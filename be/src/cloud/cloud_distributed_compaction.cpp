...
bool is_null_key_field(const PValues& column) {
    return column.has_type() && is_supported_key_pb_type(column.type().id()) &&
           column.has_null() && column.null_map_size() == 1 && column.null_map(0) &&
           column.double_value().empty() && column.float_value().empty() &&
           column.int32_value().empty() && column.int64_value().empty() &&
           column.uint32_value().empty() && column.uint64_value().empty() &&
           column.bool_value().empty() && column.string_value().empty() &&
...
void set_date_key_column(PValues* column, FieldType type, const Field& field) {
    ...
    case FieldType::OLAP_FIELD_TYPE_DATE:
        ...
        return;
    case FieldType::OLAP_FIELD_TYPE_DATETIME:
        column->add_int64_value(
                binary_cast<VecDateTimeValue, int64_t>(field.get<TYPE_DATETIME>()));
        return;
    case FieldType::OLAP_FIELD_TYPE_DATEV2:
...
Status DistributedCompactionCoordinator::start_base_compaction(
    ...
    const size_t range_count =
            cast_set<size_t>(1 + (input_rowsets_total_size - 1) / target_size);
    RETURN_IF_ERROR(try_submit_base_compaction_tasks(
            input_rowsets, output_rowset_writer, range_count, is_vertical, avg_segment_rows,
            started));
    if (!*started) {
        return Status::OK();
    }
...
Status DistributedCompactionCoordinator::validate_partial_rowset(
    ...
    if (!response.has_partial_rowset_meta()) {
        return Status::InvalidArgument("distributed single-rowset compaction result has no partial "
...
Status DistributedCompactionCoordinator::try_submit_base_compaction_tasks(
    ...
    *started = false;
    ...
    LOG_INFO("finish distributed base compaction range planning")
            ...
            .tag("short_key_encoding_lossless",
                 key_sampling_plan.short_key_encoding_lossless)
            ...
            .tag("short_key_fast_path_fallback_reason",
                 key_sampling_plan.short_key_fallback_reason)
    ...
    if (_is_mow) {
        ...
        _tablet->calc_compaction_output_rowset_delete_bitmap_by_segments(
                *_rowid_conversion, _output_rowset->rowset_id(), _output_segment_ids,
                0, request->delete_bitmap_end_version(),
...
Status DistributedCompactionWorkerManager::submit(
    ...
    auto tablet = DORIS_TRY(
            engine.tablet_mgr().get_tablet(request.output_rowset_meta().tablet_id()));
                ...
                const Status task_submit_status = worker_pool->submit_func(
                        [request_copy, batch, job_index]() {
                            ...
                            if (request_copy->output_rowset_meta().txn_expiration() <=
                                ...
                                task_status =
                                        job.worker->execute_compaction(&task_request, &job.task);
                            }
                            if (!task_status.ok()) {
