...
std::vector<CompactionWorkerInfo> select_compaction_workers_for_groups(
    ...
    std::ranges::sort(workers, [coordinator_backend_id, execution_seed](
                                         const CompactionWorkerInfo& lhs,
...
DistributedCompactionCoordinator::DistributedCompactionCoordinator(
        ...
        : _engine(engine),
          ...
          _execution_id(std::move(execution_id)) {}
...
Status DistributedCompactionCoordinator::submit_batches(
    ...
    auto token = _engine.distributed_compaction_thread_pool().new_token(
    ...
    for (size_t worker_index = 0; worker_index < workers.size(); ++worker_index) {
        submit_status = token->submit_func([&, worker_index]() {
            ...
            for (int attempt = 1; attempt <= MAX_SUBMIT_ATTEMPTS; ++attempt) {
                ...
                rpc_status = distributed_compaction_submit_rpc(
                        workers[worker_index].endpoint, requests[worker_index],
...
void DistributedCompactionCoordinator::dispatch_poll() {
    ...
    if (_engine.stopped()) {
        complete_polling(
                Status::Cancelled("storage engine stopped during distributed compaction"));
        return;
    }
    ...
    size_t rpc_count = 0;
    for (size_t worker_index = 0; worker_index < _execution_plan->workers.size();
         ++worker_index) {
    ...
    for (size_t worker_index = 0; worker_index < _execution_plan->workers.size();
        ...
        const Status submit_status =
                _engine.distributed_compaction_thread_pool().submit_func(
                        [self, round, worker_index] {
                            round->rpc_statuses[worker_index] =
                                    distributed_compaction_get_status_rpc(
                                            self->_execution_plan->workers[worker_index].endpoint,
                                            round->status_requests[worker_index],
                                            &round->status_responses[worker_index]);
                            if (round->remaining_rpcs.fetch_sub(
...
void DistributedCompactionCoordinator::finish_poll_round(
        std::shared_ptr<PollRoundContext> round) {
    DORIS_CHECK(_execution_plan != nullptr);
    DORIS_CHECK(!_execution_plan->polling_completed);
    if (_engine.stopped()) {
...
Status DistributedCompactionCoordinator::validate_partial_rowset(
    ...
    if (num_segments > task.segment_id_slot.capacity) {
        return Status::Error<ErrorCode::TOO_MANY_SEGMENTS>(
                ...
                num_segments, task.segment_id_slot.capacity);
    }
...
Status DistributedCompactionCoordinator::prepare_single_rowset(
    ...
    *started = false;
    ...
    workers = select_compaction_workers_for_groups(
            workers, BackendOptions::get_backend_id(), segment_ranges.size(), _execution_id);
    ...
    *stats = Merger::Statistics {};
    ...
    for (size_t group_index = 0; group_index < responses.size(); ++group_index) {
        ...
        RETURN_IF_ERROR(validate_partial_rowset(group_index, response, task, tablet_schema,
                                                output_rowset_writer.rowset_id(),
                                                &partial_rowset));
        ...
        if (is_mow && response.has_output_delete_bitmap_shard()) {
            _state->output_delete_bitmap->merge(
...
Status DistributedCompactionWorker::handle_compaction(
    ...
    if (segment_pos_end > input_rowset->num_segments()) {
        return Status::InvalidArgument(
                "invalid input segment range: start={}, end={}, segments={}", segment_pos_start,
                segment_pos_end, input_rowset->num_segments());
    }
    ...
    auto writer = DORIS_TRY(construct_output_rowset_writer(
            *request, *task, output_meta, *storage_resource.value()));
