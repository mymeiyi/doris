...
Status CloudStorageEngine::start_bg_threads(std::shared_ptr<WorkloadGroup> wg_sptr) {
    ...
    RETURN_IF_ERROR(ThreadPoolBuilder("CloudDistributedCompactionRpcThreadPool")
                            .set_min_threads(0)
                            .set_max_threads(
                                    config::cloud_distributed_compaction_rpc_thread_num)
                            .set_max_queue_size(
                                    config::cloud_distributed_compaction_rpc_queue_size)
    ...
    const Status retry_status = _distributed_compaction_poll_scheduler->schedule(
            ...
            [this, tablet_id, compaction, complete_task, remote_status]() mutable {
                _schedule_distributed_base_compaction_resume(
                        tablet_id, std::move(compaction), std::move(complete_task),
...
Status CloudStorageEngine::_submit_cumulative_compaction_task(const CloudTabletSPtr& tablet,
    ...
    st = submit_thread_pool->submit_func([=, this, compaction = std::move(compaction)]() {
        ...
        bool cumu_thread_acquired = false;
        Defer defer {[&]() {
            DBUG_EXECUTE_IF("CloudStorageEngine._submit_cumulative_compaction_task.sleep",
                            { sleep(5); })
            CompactionTaskTracker::instance()->remove_task(compaction_id);
            if (cumu_thread_acquired) {
                std::lock_guard lock(_cumu_compaction_delay_mtx);
                _cumu_compaction_thread_pool_used_threads--;
                if (!is_large_task) {
                    _cumu_compaction_thread_pool_small_tasks_running--;
