...
Status SegmentLoader::load_segment(const BetaRowsetSharedPtr& rowset, int64_t segment_id,
    ...
    return load_segment(rowset, rowset->rowset_meta()->position_of(segment_id), segment_id,
                        cache_handle, use_cache, need_load_pk_index_and_bf, index_load_stats);
}

Status SegmentLoader::load_segment(const BetaRowsetSharedPtr& rowset, size_t pos,
                                   int64_t segment_id, SegmentCacheHandle* cache_handle,
    ...
    RETURN_IF_ERROR(rowset->load_segment(pos, segment_id, index_load_stats, &segment));
...
Status SegmentLoader::load_segments(const BetaRowsetSharedPtr& rowset,
    ...
    for (size_t pos = 0; pos < cast_set<size_t>(rowset->num_segments()); pos++) {
        RETURN_IF_ERROR(load_segment(rowset, pos, rowset->rowset_meta()->segment_id(pos),
                                     cache_handle, use_cache, need_load_pk_index_and_bf,
                                     index_load_stats));
    }
...
void SegmentLoader::erase_segments(const RowsetMeta& rowset_meta) {
    for (size_t pos = 0; pos < cast_set<size_t>(rowset_meta.num_segments()); ++pos) {
        erase_segment(SegmentCache::CacheKey(rowset_meta.rowset_id(), rowset_meta.segment_id(pos)));
    }
}
