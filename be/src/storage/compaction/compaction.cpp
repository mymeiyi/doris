...
bool CloudCompactionMixin::should_apply_cumulative_compaction_result(
    ...
    if (response_cumulative_compaction_cnt != local_cumulative_compaction_cnt + 1) {
        // The response also reflects other compactions, but only this task's output is available
        // locally. Keep the counter behind so the next sync fetches every missing output.
