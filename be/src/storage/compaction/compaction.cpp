...
Status Compaction::execute_merge(
        const std::vector<RowsetReaderSharedPtr>& input_rs_readers, int64_t merge_way_num,
    ...
    if (_is_vertical && !_tablet->tablet_schema()->has_seq_map()) {
        ...
        status = Merger::vertical_merge_rowsets(
                ...
                _output_rs_writer.get(), cast_set<uint32_t>(get_avg_segment_rows()), merge_way_num,
                stats, progress_cb, segment_range);
    } else {
