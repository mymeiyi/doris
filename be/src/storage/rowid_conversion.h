...
public:
    ...
    Status init_segment_ranges(const std::vector<SegmentRowIdRange>& ranges) {
        ...
        for (const auto& range : ranges) {
            ...
            if (doris::GlobalMemoryArbitrator::is_exceed_hard_mem_limit(RESERVED_MEMORY)) {
                return Status::MemoryLimitExceeded("RowIdConversion init_segment_ranges failed");
            }
    ...
    void add(const std::vector<RowLocation>& rss_row_ids,
        ...
        for (auto& item : rss_row_ids) {
            ...
            if (_mode == Mode::FULL) {
                ...
                RangeMap* range_map = find_range(item.rowset_id, item.segment_id, item.row_id);
    ...
    int get(const RowLocation& src, DestinationRowId* dst) const {
        if (_mode == Mode::RANGED) {
            const RangeMap* range_map = find_range(src.rowset_id, src.segment_id, src.row_id);
