...
int64_t BaseBetaRowsetWriter::_num_seg() const {
    ...
    if (!_segment_ids.empty()) {
        return cast_set<int64_t>(_segment_ids.size());
    }
    return _num_segment;
}
...
Status BaseBetaRowsetWriter::_build_rowset_meta(RowsetMeta* rowset_meta, bool check_segment_num) {
    ...
    if (config::enable_cloud_segment_id_list && !rowset_meta->is_local()) {
        auto segment_ids = _segment_ids_by_position();
        std::vector<int64_t> persisted_segment_ids(segment_ids.begin(), segment_ids.end());
        rowset_meta->set_segment_ids(persisted_segment_ids);
    }
...
Status BaseBetaRowsetWriter::add_segment(uint32_t segment_id, const SegmentStatistics& segstat) {
    DORIS_CHECK_GE(cast_set<int64_t>(segment_id), _segment_start_id);
    ...
    {
        std::lock_guard<std::mutex> lock(_segid_statistics_map_mutex);
        DORIS_CHECK(_segid_statistics_map.find(segment_id) == _segid_statistics_map.end());
        _segid_statistics_map.emplace(segment_id, std::move(stored_segstat));
        if (config::enable_cloud_segment_id_list && !_context.is_local_rowset()) {
            _segment_ids.push_back(cast_set<int32_t>(segment_id));
        }
        _segment_num_rows.push_back(cast_set<uint32_t>(segstat.row_num));
