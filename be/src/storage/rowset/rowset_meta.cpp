...
void RowsetMeta::append_segment_ids(const google::protobuf::RepeatedField<int64_t>& segment_ids) {
...
void RowsetMeta::merge_rowset_meta(const RowsetMeta& other) {
    ...
    if (has_segment_ids() || other.has_segment_ids()) {
        ...
        // persist its own segment_ids -- e.g. enable_cloud_segment_id_list was off while writing
        // `other`
        ...
        if (!has_segment_ids()) {
            for (int64_t pos = 0; pos < old_num_segments; ++pos) {
                _rowset_meta_pb.add_segment_ids(pos);
            }
        }
        if (other.has_segment_ids()) {
            ...
            for (int64_t k = 0; k < other.num_segments(); ++k) {
                _rowset_meta_pb.add_segment_ids(appended_start + k);
            }
        }
        set_num_segments(_rowset_meta_pb.segment_ids_size());
