...
TEST(CloudDistributedCompactionTest, expands_key_prefix_to_improve_range_balance) {
    const std::vector<cloud::CompositeKeySample> samples = {
            {.key = {Field::create_field<TYPE_INT>(0), Field::create_field<TYPE_INT>(1)},
             .weight = 15},
            {.key = {Field::create_field<TYPE_INT>(0), Field::create_field<TYPE_INT>(2)},
             .weight = 15},
            {.key = {Field::create_field<TYPE_INT>(0), Field::create_field<TYPE_INT>(3)},
             .weight = 15},
            {.key = {Field::create_field<TYPE_INT>(1), Field::create_field<TYPE_INT>(1)},
             .weight = 10},
            {.key = {Field::create_field<TYPE_INT>(1), Field::create_field<TYPE_INT>(2)},
             .weight = 10},
            {.key = {Field::create_field<TYPE_INT>(1), Field::create_field<TYPE_INT>(3)},
             .weight = 10},
            {.key = {Field::create_field<TYPE_INT>(2), Field::create_field<TYPE_INT>(1)},
             .weight = 25}};

    const auto plan = cloud::choose_composite_key_range_boundaries(samples, 3);
    ASSERT_EQ(plan.prefix_length, 2);
    ASSERT_EQ(plan.boundaries.size(), 2);
    EXPECT_EQ(plan.boundaries[0][0].get<TYPE_INT>(), 0);
    EXPECT_EQ(plan.boundaries[0][1].get<TYPE_INT>(), 3);
    EXPECT_EQ(plan.boundaries[1][0].get<TYPE_INT>(), 1);
    EXPECT_EQ(plan.boundaries[1][1].get<TYPE_INT>(), 3);
}
...
TEST(CloudDistributedCompactionTest, validates_distributed_compaction_partial_rowsets) {
    ...
    cloud::DistributedCompactionTask task {
            .worker_endpoint = "",
            .group_index = 0,
    ...
    auto validate = [&](const PCloudDistributedCompactionTaskResult& response) {
        ...
        return cloud::validate_distributed_compaction_partial_rowset(
                0, response, task, tablet_schema, output_rowset_id, &partial_meta, &key_bounds,
                &segment_rows);
    };
    ...
    auto make_two_segment_response = [&](std::string_view first_min, std::string_view first_max,
                                         std::string_view second_min, std::string_view second_max,
                                         bool key_bounds_truncated = false) {
        auto response = make_valid_response();
        auto* meta = response.mutable_partial_rowset_meta();
        meta->set_num_segments(2);
        meta->add_segment_ids(11);
        meta->mutable_segments_key_bounds(0)->set_min_key(first_min.data(), first_min.size());
        meta->mutable_segments_key_bounds(0)->set_max_key(first_max.data(), first_max.size());
        auto* second_bounds = meta->add_segments_key_bounds();
        second_bounds->set_min_key(second_min.data(), second_min.size());
        second_bounds->set_max_key(second_max.data(), second_max.size());
        meta->add_num_segment_rows(1);
        meta->set_segments_key_bounds_truncated(key_bounds_truncated);
        return response;
    };
    ...
    task.encoded_lower_key = "a";
    task.encoded_upper_key = "e";
    EXPECT_TRUE(validate(make_two_segment_response("a", "b", "c", "d")).ok());
    ...
    EXPECT_TRUE(validate(make_two_segment_response("a", "b", "c", "d", true)).ok());
    task.encoded_lower_key = "b";
    EXPECT_FALSE(validate(make_two_segment_response("a", "b", "c", "d", true)).ok());
    task.encoded_lower_key.reset();
    task.encoded_upper_key = "d";
    EXPECT_FALSE(validate(make_two_segment_response("a", "b", "c", "d", true)).ok());
}
...
TEST(CloudDistributedCompactionTest, grouped_cumulative_global_lock_failure_cleans_up_task) {
    CloudStorageEngine engine(EngineOptions {});
    auto tablet_meta = create_compaction_tablet_meta();
    ...
    rowset_meta->set_num_segments(4);
    rowset_meta->set_segments_overlap(OVERLAPPING);
    rowset_meta->set_total_disk_size(1024);
    rowset_meta->set_tablet_schema(tablet->tablet_schema());
    RowsetSharedPtr rowset;
    ASSERT_TRUE(RowsetFactory::create_rowset(nullptr, "", rowset_meta, &rowset).ok());
    compaction->_input_rowsets = {rowset};
    compaction->_single_rowset_compaction_segment_group_size = 2;
    ...
    Defer shutdown_pool {[&] { engine._cumu_compaction_thread_pool->shutdown(); }};
...
TEST(CloudDistributedCompactionTest, grouped_cumulative_large_task_delay_keeps_counts_balanced) {
    CloudStorageEngine engine(EngineOptions {});
    ...
    Defer restore_config {[&] {
        ...
        engine._cumu_compaction_thread_pool_used_threads = 0;
    }};
    ...
    ASSERT_TRUE(ThreadPoolBuilder("GroupedCumuCompactionDelayTest")
                        ...
                        .build(&engine._cumu_compaction_thread_pool)
                        .ok());
    engine._cumu_compaction_thread_pool_used_threads = 1;
    ...
    auto status = engine._try_submit_cumulative_compaction_task(
            tablet, compaction, compaction->compaction_id(), [&] { ++erase_submitted_calls; },
            [&] {
                std::lock_guard lock(engine._compaction_mtx);
                engine._executing_cumu_compactions.erase(tablet->tablet_id());
                ++erase_executing_calls;
            });
    ASSERT_TRUE(status.has_value());
    ASSERT_TRUE(status->ok()) << status->to_string();
    ...
    EXPECT_EQ(engine._cumu_compaction_thread_pool_used_threads, 1);
