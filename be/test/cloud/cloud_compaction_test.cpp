...
TEST_F(CloudCompactionTest, distributed_base_compaction_rejects_zorder) {
    const auto old_enable = config::enable_cloud_distributed_base_compaction;
    const auto old_target = config::cloud_distributed_base_compaction_target_range_input_size_bytes;
    ...
    auto rowset = create_rowset(Version(0, 0), 1, false, 2);
    ASSERT_NE(rowset, nullptr);
    TabletSchemaPB schema_pb;
    rowset->tablet_schema()->to_schema_pb(&schema_pb);
    _tablet_meta->mutable_tablet_schema()->init_from_pb(schema_pb);
    ...
    ASSERT_TRUE(compaction.can_use_distributed_base_compaction());

    schema_pb.set_sort_type(SortType::ZORDER);
    _tablet_meta->mutable_tablet_schema()->init_from_pb(schema_pb);
    EXPECT_FALSE(compaction.can_use_distributed_base_compaction());
}
