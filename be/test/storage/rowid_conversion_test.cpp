...
TEST_F(TestRowIdConversion, SingleRowsetGroupedCompactionRowIdConversionIsComplete) {
    ...
    for (bool is_vertical : {false, true}) {
        ...
        for (size_t segment_pos = 0; segment_pos < input_data.size(); ++segment_pos) {
            ...
            EXPECT_EQ(input_segment_id,
                      input_segment_start_id + cast_set<int64_t>(segment_pos));
            for (int64_t row_id = 0; row_id < rows_per_segment; ++row_id) {
                ...
                for (uint32_t output_segment_pos = 0; output_segment_pos < dst.segment_pos;
                     ++output_segment_pos) {
        ...
        if (is_vertical) {
            ...
            for (size_t segment_pos = 0; segment_pos < second_output_segments.size();
                ...
                EXPECT_LE(segment->min_key(), segment->max_key())
                        << "segment_pos=" << segment_pos;
                if (segment_pos > 0) {
