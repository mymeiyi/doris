...
TEST_F(RowsetMetaTest, TestSegmentIdsAccessors) {
    ...
    // Non-contiguous segment_ids: position <-> real id mapping and inferred next_segment_id.
...
// (enable_cloud_segment_id_list on while writing it). Merge must concatenate the real ids.
...
// (enable_cloud_segment_id_list off while writing it) but `base` already carries a
