...
## 10. 单 BE 多 task CPU 核占用观测

### 10.1 方法和输入

2026-09-03 使用日志表模型额外执行一轮并行 Base，专门验证同一 BE 上 8 个 task 的 CPU
占用。测试表为 `ali_virginia_prod_06_core_n8_20260903`，输入 20,000,000 行、20 个
single-segment `NONOVERLAPPING` rowset，总大小 12,729,698,984 B（11.85 GiB）。range
target 为 512 MiB，实际生成 24 个 range，并按 8/8/8 分配到 3 个 BE。

在触发前 20 秒到结束后 10 秒，以 1 秒粒度同时采集：

- `top -1 -H`：观察每个 CPU 和 worker 线程的瞬时利用率。
- `pidstat`：观察 BE 进程及每个线程的 CPU 时间。
- worker 日志：按 endpoint 汇总 `cpu_time_us`、`worker_start_time_us` 和
  `worker_finish_time_us`。

### 10.2 结果

Compaction profile 耗时 28,075 ms，端到端吞吐 432.41 MiB/s；输入全部命中文件缓存，
remote read 为 0，三个 BE 的平均 iowait 均低于 0.7%。

| BE | task | task CPU 合计 | worker 窗口 | 日志核等价值 | task CPU/elapsed | pidstat task 线程 | pidstat 进程增量 | mpstat 整机增量 |
| --- | ---: | ---: | ---: | ---: | ---: | --- | ---: | ---: |
| 172.20.49.5 | 8 | 205.09 s | 27.22 s | 7.54 核 | 97.1% | 8 条，95.7%～98.0% | 7.87 核 | 7.99 核 |
| 172.20.49.6 | 8 | 195.37 s | 26.38 s | 7.41 核 | 95.5% | 8 条，94.0%～97.4% | 8.63 核 | 8.77 核 |
| 172.20.49.7 | 8 | 187.80 s | 25.52 s | 7.36 核 | 94.3% | 8 条，92.0%～96.0% | 9.13 核 | 9.23 核 |

`top -H` 在 15:46:40 的同一个采样点也在每台 BE 上看到恰好 8 条 RUNNING 的
`CloudDi+` task 线程，每条约 94%～100%。线程会在 CPU 间迁移，因此单个 CPU 的瞬时
利用率并不固定；8 个物理核对应的 8 组 SMT sibling 在执行窗口内均出现过高负载。

日志核等价值只统计 8 条 task 主线程；`pidstat` 和 `mpstat` 还包含索引、压缩、文件写出
等辅助线程，所以进程/整机增量可以略高于 8 核。三种观测结论一致：在本轮全缓存、倒排
索引密集的 CPU-bound 负载中，同一 BE 的 8 个 distributed task 确实并发运行，并持续
使用约 7.4～8 个逻辑 CPU；但不能据此推导 IO-bound 表上的任意 n 个 task 都会占满 n 个核。

Compaction 后行数、非 NULL `app` 数、时间范围、`SUM(LENGTH(msg))` 和内容 fingerprint
均与母表完全一致；输出为一个包含 24 个 `NONOVERLAPPING` segment 的 `[2-21]` rowset。
