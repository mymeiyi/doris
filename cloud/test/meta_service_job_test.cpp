...
TEST(MetaServiceJobTest, DeleteBitmapUpdateLockCompatibilityTest) {
    ...
    // test compaction and schema change
    {
        ...
        remove_delete_bitmap_lock(meta_service.get(), table_id);
        // 1. compaction1 get lock but not commit job and the lock is timeout
        auto tablet_id = 5;
        ...
        sleep(2);
        // 2. schema change1 get lock (timeout; or commit)
        ASSERT_NO_FATAL_FAILURE(create_tablet(meta_service.get(), table_id, 2, 3,
        ...
        ASSERT_EQ(res.status().code(), MetaServiceCode::LOCK_EXPIRED); // error
        // v1: lock initiator 12345 not exist
