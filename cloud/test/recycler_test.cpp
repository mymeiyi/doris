...
TEST(RecyclerTest, recycle_tmp_rowsets_deletes_committed_txn_tmp_data) {
    // This test reproduces a race condition where recycle_tmp_rowsets deletes
    // tmp rowsets that are still needed by a COMMITTED (but not yet VISIBLE) txn
    // in the lazy commit path.
    //
    // Scenario:
    //   1. A txn goes through lazy commit: status=COMMITTED, tmp rowset keys exist
    //   2. recycle_tmp_rowsets runs and deletes expired tmp rowsets (only checks time, not txn status)
    //   3. convert_tmp_rowsets runs later, finds tmp keys missing, assumes "already converted"
    //   4. Txn becomes VISIBLE but rowset data is gone -> data loss

    auto old_retention = config::retention_seconds;
    auto old_force = config::force_immediate_recycle;
    DORIS_CLOUD_DEFER {
        config::retention_seconds = old_retention;
        config::force_immediate_recycle = old_force;
    };
    config::retention_seconds = 0;
    ...
    instance.set_multi_version_status(MULTI_VERSION_WRITE_ONLY);
    auto obj_info = instance.add_obj_info();
    obj_info->set_id("recycle_tmp_committed_txn");
    obj_info->set_ak(config::test_s3_ak);
    obj_info->set_sk(config::test_s3_sk);
    obj_info->set_endpoint(config::test_s3_endpoint);
    obj_info->set_region(config::test_s3_region);
    obj_info->set_bucket(config::test_s3_bucket);
    obj_info->set_prefix("recycle_tmp_committed_txn");
    ...
    constexpr int64_t test_txn_id = 99001;
    constexpr int64_t test_tablet_id = 20001;
    constexpr int64_t test_index_id = 30001;

    doris::TabletSchemaCloudPB schema;
    schema.set_schema_version(0);

    auto rowset = create_rowset("recycle_tmp_committed_txn", test_tablet_id, test_index_id, 3,
                                schema, test_txn_id);

    // Step 1: Create tmp rowset (simulates what commit_rowset does)
    create_tmp_rowset(txn_kv.get(), accessor.get(), rowset, false, false, false);

    // Step 2: Write a COMMITTED txn info (simulates lazy commit's first phase)
    {
        TxnInfoPB txn_info;
        txn_info.set_db_id(db_id);
        txn_info.set_txn_id(test_txn_id);
        txn_info.set_status(TxnStatusPB::TXN_STATUS_COMMITTED);
        txn_info.set_label("test_lazy_commit_label");

        std::string key = txn_info_key({instance_id, db_id, test_txn_id});
        std::unique_ptr<Transaction> txn;
        ASSERT_EQ(txn_kv->create_txn(&txn), TxnErrorCode::TXN_OK);
    ...
    // Verify tmp rowset and segment files exist before recycling
    {
        std::unique_ptr<Transaction> txn;
        ASSERT_EQ(txn_kv->create_txn(&txn), TxnErrorCode::TXN_OK);
        std::string tmp_key = meta_rowset_tmp_key({instance_id, test_txn_id, test_tablet_id});
        std::string val;
        ASSERT_EQ(txn->get(tmp_key, &val), TxnErrorCode::TXN_OK)
                << "tmp rowset key should exist before recycle";
    }
    for (int i = 0; i < rowset.num_segments(); ++i) {
        auto path = segment_path(test_tablet_id, rowset.rowset_id_v2(), i);
        EXPECT_EQ(accessor->exists(path), 1) << "segment file should exist before recycle: " << path;
    }

    // Step 3: recycle_tmp_rowsets deletes the expired tmp rowsets
    // (retention_seconds=0 so everything is immediately expired)
    ASSERT_EQ(recycler.recycle_tmp_rowsets(), 0);
    ASSERT_EQ(recycler.recycle_tmp_rowsets(), 0); // Two passes for mark-delete if enabled

    // Verify tmp rowset key is deleted
    {
        std::unique_ptr<Transaction> txn;
        ASSERT_EQ(txn_kv->create_txn(&txn), TxnErrorCode::TXN_OK);
        std::string tmp_key = meta_rowset_tmp_key({instance_id, test_txn_id, test_tablet_id});
        ...
        ASSERT_EQ(txn->get(tmp_key, &val), TxnErrorCode::TXN_KEY_NOT_FOUND)
                << "tmp rowset key should be deleted after recycle";
    }

    // Verify segment files are deleted
    for (int i = 0; i < rowset.num_segments(); ++i) {
        auto path = segment_path(test_tablet_id, rowset.rowset_id_v2(), i);
        EXPECT_EQ(accessor->exists(path), 0) << "segment file should be deleted after recycle: " << path;
    }

    // Step 4: Simulate what convert_tmp_rowsets does when processing this txn's tmp rowsets
    // This is the core of the bug: when tmp key is not found, it assumes "already converted"
    // and silently skips, instead of detecting the data loss.
    {
        std::unique_ptr<Transaction> txn;
        ASSERT_EQ(txn_kv->create_txn(&txn), TxnErrorCode::TXN_OK);
        std::string tmp_key = meta_rowset_tmp_key({instance_id, test_txn_id, test_tablet_id});
        std::string tmp_data;
        TxnErrorCode err = txn->get(tmp_key, &tmp_data);

        // This is exactly what convert_tmp_rowsets does at line 264-267 of txn_lazy_committer.cpp
        if (TxnErrorCode::TXN_KEY_NOT_FOUND == err) {
            // BUG: the code assumes "the tmp rowset has been converted" but actually
            // it was deleted by recycle_tmp_rowsets. The rowset data is lost.
            LOG(INFO) << "BUG REPRODUCED: tmp rowset key not found, code assumes 'already converted' "
                      << "but data was actually deleted by recycler. txn_id=" << test_txn_id;
        }
        ASSERT_EQ(err, TxnErrorCode::TXN_KEY_NOT_FOUND)
                << "tmp rowset key should be gone, confirming the race condition";
    }

    // Verify: the txn is still COMMITTED (not yet VISIBLE), but its tmp rowset data is gone.
    // If convert_tmp_rowsets were to run now, it would skip this rowset and the txn would
    // become VISIBLE without the actual rowset data -> data loss.
    {
        std::unique_ptr<Transaction> txn;
        ASSERT_EQ(txn_kv->create_txn(&txn), TxnErrorCode::TXN_OK);
        std::string info_key = txn_info_key({instance_id, db_id, test_txn_id});
        std::string info_val;
        ASSERT_EQ(txn->get(info_key, &info_val), TxnErrorCode::TXN_OK);
        TxnInfoPB txn_info;
        ASSERT_TRUE(txn_info.ParseFromString(info_val));
        ASSERT_EQ(txn_info.status(), TxnStatusPB::TXN_STATUS_COMMITTED)
                << "txn should still be COMMITTED";
    }

    // Verify no formal rowset was created (because convert_tmp_rowsets never ran)
    {
        std::unique_ptr<Transaction> txn;
        ASSERT_EQ(txn_kv->create_txn(&txn), TxnErrorCode::TXN_OK);
        // A formal rowset key would be meta_rowset_key({instance_id, tablet_id, version})
        // Since convert_tmp_rowsets didn't run, no formal rowset should exist
        std::string rowset_key = meta_rowset_key({instance_id, test_tablet_id, 2}); // version 2
        std::string rowset_val;
        ASSERT_EQ(txn->get(rowset_key, &rowset_val), TxnErrorCode::TXN_KEY_NOT_FOUND)
                << "no formal rowset should exist because convert_tmp_rowsets didn't run";
    }
}

TEST(RecyclerTest, recycle_tmp_committed_txn_end_to_end_data_loss) {
    // End-to-end test: demonstrates the FULL data loss lifecycle.
    //
    // The existing test (above) only verifies the intermediate state (COMMITTED + data gone).
    // This test goes further: simulates convert_tmp_rowsets running after the recycler deleted
    // the tmp keys, then makes the txn VISIBLE, and proves the rowset data is permanently lost.
    //
    // Race timeline:
    //   T1: Lazy commit phase 1 — txn=COMMITTED, tmp rowset keys + segment files written
    //   T2: recycle_tmp_rowsets — deletes expired tmp rowsets (only checks time)
    //   T3: Lazy commit phase 2 — convert_tmp_rowsets finds TXN_KEY_NOT_FOUND,
    //       assumes "already converted", skips silently
    //   T4: make_committed_txn_visible — txn becomes VISIBLE
    //   T5: Read path — no formal rowset, no segment files → DATA LOSS
    ...
    config::retention_seconds = 0; // everything expires immediately
    ...
    instance.set_multi_version_status(MULTI_VERSION_WRITE_ONLY);
    auto obj_info = instance.add_obj_info();
    obj_info->set_id("e2e_data_loss");
    obj_info->set_ak(config::test_s3_ak);
    obj_info->set_sk(config::test_s3_sk);
    obj_info->set_endpoint(config::test_s3_endpoint);
    obj_info->set_region(config::test_s3_region);
    obj_info->set_bucket(config::test_s3_bucket);
    obj_info->set_prefix("e2e_data_loss");
    ...
    constexpr int64_t test_txn_id = 99002;
    constexpr int64_t test_tablet_id = 20002;
    constexpr int64_t test_index_id = 30002;

    doris::TabletSchemaCloudPB schema;
    schema.set_schema_version(0);

    auto rowset = create_rowset("e2e_data_loss", test_tablet_id, test_index_id, 3, schema,
                                test_txn_id);

    // ============================================================
    // T1: Simulate lazy commit phase 1 — write tmp rowset + COMMITTED txn info
    // ============================================================
    create_tmp_rowset(txn_kv.get(), accessor.get(), rowset, false, false, false);

    // Write COMMITTED txn info
    {
        TxnInfoPB txn_info;
        txn_info.set_db_id(db_id);
        txn_info.set_txn_id(test_txn_id);
        txn_info.set_status(TxnStatusPB::TXN_STATUS_COMMITTED);
        txn_info.set_label("test_e2e_data_loss_label");
    ...
    // Pre-verify: tmp key exists, segment files exist
    {
        std::unique_ptr<Transaction> txn;
        ASSERT_EQ(txn_kv->create_txn(&txn), TxnErrorCode::TXN_OK);
        std::string tmp_key = meta_rowset_tmp_key({instance_id, test_txn_id, test_tablet_id});
    ...
    for (int i = 0; i < rowset.num_segments(); ++i) {
        auto path = segment_path(test_tablet_id, rowset.rowset_id_v2(), i);
        ASSERT_EQ(accessor->exists(path), 1) << "segment should exist before recycle";
    }

    // ============================================================
    // T2: recycle_tmp_rowsets — deletes expired tmp rowsets
    // ============================================================
    ASSERT_EQ(recycler.recycle_tmp_rowsets(), 0);
    ASSERT_EQ(recycler.recycle_tmp_rowsets(), 0); // two passes if mark-delete enabled

    // Verify: tmp key and segment files are gone
    {
        std::unique_ptr<Transaction> txn;
        ASSERT_EQ(txn_kv->create_txn(&txn), TxnErrorCode::TXN_OK);
        std::string tmp_key = meta_rowset_tmp_key({instance_id, test_txn_id, test_tablet_id});
        std::string val;
        ASSERT_EQ(txn->get(tmp_key, &val), TxnErrorCode::TXN_KEY_NOT_FOUND)
                << "tmp key should be deleted by recycler";
    }
    for (int i = 0; i < rowset.num_segments(); ++i) {
        auto path = segment_path(test_tablet_id, rowset.rowset_id_v2(), i);
        ASSERT_EQ(accessor->exists(path), 0) << "segment should be deleted by recycler";
    }

    // ============================================================
    // T3: Simulate convert_tmp_rowsets (lazy commit phase 2)
    //     This is exactly what txn_lazy_committer.cpp:264-267 does:
    //     TXN_KEY_NOT_FOUND → assume "already converted" → continue
    // ============================================================
    bool convert_skipped_missing_key = false;
    {
        std::unique_ptr<Transaction> txn;
        ASSERT_EQ(txn_kv->create_txn(&txn), TxnErrorCode::TXN_OK);
        std::string tmp_key = meta_rowset_tmp_key({instance_id, test_txn_id, test_tablet_id});
        ...
        // This is the silent skip in convert_tmp_rowsets:
        //   if (TXN_KEY_NOT_FOUND == err) {
        //       VLOG_DEBUG << "tmp rowset has been converted";
        //       continue;  // <-- BUG: data was deleted, not converted
        //   }
        // No error raised, no abort, no retry. The function proceeds to commit
        // the txn as if everything is fine.
        convert_skipped_missing_key = true;
    }
    ASSERT_TRUE(convert_skipped_missing_key)
            << "convert_tmp_rowsets silently skipped the missing tmp key";

    // ============================================================
    // T4: Make txn VISIBLE (simulate make_committed_txn_visible)
    // ============================================================
    {
        std::unique_ptr<Transaction> txn;
        ASSERT_EQ(txn_kv->create_txn(&txn), TxnErrorCode::TXN_OK);
    ...
    // 5a: Txn is VISIBLE
    {
        ...
        ASSERT_EQ(txn_info.status(), TxnStatusPB::TXN_STATUS_VISIBLE)
                << "txn has been made VISIBLE";
    }

    // 5b: No formal rowset exists (convert_tmp_rowsets never actually wrote one,
    //     because it skipped the missing tmp key and had nothing to convert)
    {
        std::unique_ptr<Transaction> txn;
        ASSERT_EQ(txn_kv->create_txn(&txn), TxnErrorCode::TXN_OK);
        // A formal rowset key would be meta_rowset_key({instance_id, tablet_id, version}).
        // Since the tmp key was missing, convert_tmp_rowsets skipped this rowset entirely
        // and never wrote a formal rowset.
        std::string rowset_key = meta_rowset_key({instance_id, test_tablet_id, 2});
        std::string rowset_val;
        ASSERT_EQ(txn->get(rowset_key, &rowset_val), TxnErrorCode::TXN_KEY_NOT_FOUND)
                << "DATA LOSS: txn is VISIBLE but no formal rowset exists";
    }

    // 5c: Segment files are gone
    for (int i = 0; i < rowset.num_segments(); ++i) {
        auto path = segment_path(test_tablet_id, rowset.rowset_id_v2(), i);
        ASSERT_EQ(accessor->exists(path), 0)
                << "DATA LOSS: segment file is gone, txn_id=" << test_txn_id
                << " path=" << path;
    }

    // 5d: No tmp rowset key either (deleted by recycler)
    {
        std::unique_ptr<Transaction> txn;
        ASSERT_EQ(txn_kv->create_txn(&txn), TxnErrorCode::TXN_OK);
        std::string tmp_key = meta_rowset_tmp_key({instance_id, test_txn_id, test_tablet_id});
    ...
    // SUMMARY: The txn is VISIBLE, but ALL data is lost:
    //   - No tmp rowset key (deleted by recycler)
    //   - No formal rowset key (convert_tmp_rowsets skipped it)
    //   - No segment files (deleted by recycler)
    //
    // Root cause: recycle_tmp_rowsets only checks expiration time, not txn status.
    // A COMMITTED (but not yet VISIBLE) txn's tmp rowsets can expire and be deleted
    // before convert_tmp_rowsets gets a chance to convert them.
}
} // namespace doris::cloud
