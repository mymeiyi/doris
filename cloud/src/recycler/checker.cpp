    ...
    while (it == nullptr /* may be not init */ || (it->more() && !stopped())) {
        ...
        while (it->has_next() && !stopped()) {
            ...
            if (tablet_rowsets_cache.tablet_id == -1 ||
                tablet_rowsets_cache.tablet_id != tablet_id) {
                // Delete bitmaps are scanned in ascending tablet_id order, so once we move on to
                // a new tablet the previous one is never revisited. Drop its tmp rowset entry to
                // bound memory during the scan (tablets that own tmp rowsets but no delete bitmap
                // are not freed here, but they are released when this function returns).
                if (tablet_rowsets_cache.tablet_id != -1) {
