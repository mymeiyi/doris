...
void MetaServiceImpl::update_delete_bitmap(google::protobuf::RpcController* controller,
    ...
    if (store_version == 1) {
        ...
        for (size_t i = 0; i < request->rowset_delete_bitmaps_size(); ++i) {
            MetaDeleteBitmapInfoV2 key_info {instance_id, tablet_id,
                                             request->rowset_delete_bitmaps(i).rowset_id()};
            std::string key;
    ...
    LOG(INFO) << "sout: size=" << delete_bitmap_keys.delete_bitmap_keys().size()
              << " tablet_id=" << tablet_id << " store_version=" << store_version;
    for (size_t i = 0; i < delete_bitmap_keys.delete_bitmap_keys().size(); ++i) {
        auto& key = delete_bitmap_keys.delete_bitmap_keys(i);
        std::string delete_bitmap_val;
        if (store_version == 2) {
            if (!request->rowset_delete_bitmaps(i).delete_bitmap().SerializeToString(
                        &delete_bitmap_val)) {
                code = MetaServiceCode::PROTOBUF_SERIALIZE_ERR;
            ...
            LOG(INFO) << "put delete bitmap kv, tablet_id=" << tablet_id
                      << " rowset_id=" << request->rowset_delete_bitmaps(i).rowset_id()
                      << ", delete_bitmap_size="
                      << request->rowset_delete_bitmaps(i).delete_bitmap().rowset_ids_size()
                      << ". key=" << hex(key) << ", value_size=" << delete_bitmap_val.size();
        }
        auto& val = store_version == 1 ? request->segment_delete_bitmaps(i) : delete_bitmap_val;
...
void MetaServiceImpl::get_delete_bitmap(google::protobuf::RpcController* controller,
    ...
    if (store_version == 1) {
        if (rowset_ids.size() != begin_versions.size() ||
            rowset_ids.size() != end_versions.size()) {
            code = MetaServiceCode::INVALID_ARGUMENT;
    ...
    for (size_t i = 0; i < rowset_ids.size(); i++) {
        ...
        if (store_version == 1) {
            ...
            if (!val_buf.to_pb(&delete_bitmap_pb)) {
                code = MetaServiceCode::PROTOBUF_PARSE_ERR;
                ss << "failed to parse DeleteBitmapPB" << " for tablet=" << tablet_id
                   << ", rowset=" << rowset_ids[i] << ", key=" << hex(key)
                   << ", value_size=" << val_buf.value().size();
                msg = ss.str();
                return;
            }
            response->add_delete_bitmap()->CopyFrom(delete_bitmap_pb);
            LOG(INFO) << "get delete bitmap for tablet=" << tablet_id
                      << ", rowset=" << rowset_ids[i]
                      << ", delete_bitmap_num=" << delete_bitmap_pb.rowset_ids_size()
                      /*<< ", delete_bitmap_byte=" << delete_bitmap_pb*/;
        }
    }
